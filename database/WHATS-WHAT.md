...
Source of truth: `lib/tcj-category-copy.js` (A–K) + `lib/tcj-garden-taxonomy.js` (A) + `lib/tcj-feather-flock-taxonomy.js` (B) + `lib/tcj-pasture-hoof-taxonomy.js` (C) + `lib/tcj-ocean-river-taxonomy.js` (D) + `sql/fix-sips-drinks-taxonomy.sql` (Sips).
