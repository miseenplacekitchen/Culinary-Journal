-- fix-dish-index-list-filter.sql — Step 4a: Archived-only filter via p_active_filter.
-- Run once in Supabase SQL Editor after fix-dish-index-ops.sql. Safe to re-run.
-- Then run fix-dish-index-list-filter-csv.sql (step 4b), then fix-dish-index-phase-abc.sql (step 5).
...
SELECT 'fix-dish-index-list-filter (list RPC) complete' AS status;
