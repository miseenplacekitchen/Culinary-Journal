...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'admin_create_notification' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.admin_create_notification(uuid,text,uuid,text,text) TO authenticated;
...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'get_my_collections' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'upsert_collection' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
BEGIN
  ...
  ELSE
    UPDATE public.collections SET
      name=p_name, description=p_description, emoji=p_emoji, is_public=p_is_public
    WHERE id=p_id AND user_id=auth.uid() RETURNING * INTO result;
  END IF;
  RETURN result;
END;
$$;
GRANT EXECUTE ON FUNCTION public.upsert_collection(uuid,text,text,text,boolean) TO authenticated;

DO $$ DECLARE r record;
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'delete_collection' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'add_to_collection' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'not_authenticated'; END IF;
  IF NOT EXISTS (SELECT 1 FROM public.collections WHERE id=p_collection_id AND user_id=auth.uid())
    THEN RAISE EXCEPTION 'not_authorized'; END IF;
  INSERT INTO public.collection_recipes (collection_id, recipe_id)
  VALUES (p_collection_id, p_recipe_id) ON CONFLICT DO NOTHING;
END;
$$;
...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'remove_from_collection' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
...
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'get_collection_recipes' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
CREATE OR REPLACE FUNCTION public.get_collection_recipes(p_collection_id uuid)
RETURNS TABLE (
  id             uuid,
  recipe_name    text,
  category       text,
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.get_collection_recipes(uuid) TO anon, authenticated;

DO $$ DECLARE r record;
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'get_recipe_collections' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
CREATE OR REPLACE FUNCTION public.get_recipe_collections(p_recipe_id uuid)
...
BEGIN
  ...
  RETURN QUERY
    SELECT c.id, c.name, c.emoji,
           EXISTS(SELECT 1 FROM public.collection_recipes cr
                  WHERE cr.collection_id=c.id AND cr.recipe_id=p_recipe_id)
    FROM public.collections c WHERE c.user_id=auth.uid() ORDER BY c.name;
END;
$$;
GRANT EXECUTE ON FUNCTION public.get_recipe_collections(uuid) TO authenticated;
  ...
  IF p_id IS NULL THEN
    INSERT INTO public.family_profiles
      (user_id, name, relationship, age_group, allergies, spice_preference,
       dietary_needs, health_conditions, notes)
    VALUES
      (auth.uid(), p_name, p_relationship, p_age_group, p_allergies,
       p_spice_preference, p_dietary_needs, p_health_conditions, p_notes)
    RETURNING * INTO result;
  ELSE
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.upsert_family_profile(uuid,text,text,text,jsonb,text,jsonb,text[],text) TO authenticated;

DO $$ DECLARE r record;
BEGIN
  FOR r IN SELECT oid::regprocedure AS sig FROM pg_proc
           WHERE proname = 'delete_family_profile' AND pronamespace = 'public'::regnamespace
  LOOP EXECUTE 'DROP FUNCTION IF EXISTS ' || r.sig; END LOOP;
END $$;
CREATE OR REPLACE FUNCTION public.delete_family_profile(p_id uuid)
RETURNS void
LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $$
BEGIN
...
END;
$$;
GRANT EXECUTE ON FUNCTION public.delete_family_profile(uuid) TO authenticated;
