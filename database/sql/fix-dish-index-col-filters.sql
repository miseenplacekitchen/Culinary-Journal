-- fix-dish-index-col-filters.sql — Dish Index per-column header filters (server-side)
-- Run after fix-dish-index-table-controls.sql (step 9). Safe to re-run.

CREATE OR REPLACE FUNCTION public.rnl_match_col_filter(
  rnl public.recipe_name_library,
  p_linked_name text,
  p_key text,
  p_val text
)
...
AS $$
  SELECT CASE
    WHEN btrim(COALESCE(p_val, '')) = '' THEN true
    WHEN p_key = 'dish_code' THEN COALESCE(rnl.dish_code, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'recipe_name' THEN COALESCE(rnl.recipe_name, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'native_name' THEN COALESCE(rnl.native_name, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'category' THEN COALESCE(rnl.category, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'sub_category' THEN COALESCE(rnl.sub_category, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'division' THEN COALESCE(rnl.division, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'origin_country' THEN COALESCE(rnl.origin_country, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'origin_state' THEN COALESCE(rnl.origin_state, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'source_type' THEN COALESCE(rnl.source_type, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'visibility' THEN COALESCE(rnl.visibility, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'research_status' THEN COALESCE(rnl.research_status, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'content_status' THEN COALESCE(rnl.content_status, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'difficulty' THEN COALESCE(rnl.difficulty, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'spice_level' THEN COALESCE(rnl.spice_level, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'sweet_level' THEN COALESCE(rnl.sweet_level, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'cooking_style' THEN COALESCE(rnl.cooking_style, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'prep_time_minutes' THEN COALESCE(rnl.prep_time_minutes::text, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'cook_time_minutes' THEN COALESCE(rnl.cook_time_minutes::text, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'additional_time_minutes' THEN COALESCE(rnl.additional_time_minutes::text, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'servings' THEN COALESCE(rnl.servings::text, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'linked_recipe_name' THEN COALESCE(p_linked_name, '') ILIKE '%' || p_val || '%'
    WHEN p_key = 'primary_ingredients' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.primary_ingredients, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'meal_type_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.meal_type_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'occasion_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.occasion_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'style_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.style_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'flavor_profile_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.flavor_profile_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'dietary_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.dietary_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    WHEN p_key = 'health_tags' THEN EXISTS (
      SELECT 1 FROM unnest(COALESCE(rnl.health_tags, '{}')) t WHERE t ILIKE '%' || p_val || '%'
    )
    ELSE true
  END;
$$;
...
CREATE OR REPLACE FUNCTION public.admin_list_recipe_name_library(
  ...
  p_col_filters jsonb DEFAULT '{}'::jsonb
)
RETURNS json
...
BEGIN
  ...
  v_filters := COALESCE(p_col_filters, '{}'::jsonb);
  ...
  SELECT count(*) INTO v_total
   ...
   WHERE (
     ...
     AND (
          v_filters = '{}'::jsonb
          OR NOT EXISTS (
            SELECT 1 FROM jsonb_each_text(v_filters) kv
             WHERE btrim(kv.value) <> ''
               AND NOT public.rnl_match_col_filter(rnl, sr.recipe_name, kv.key, kv.value)
          )
        );
      ...
      FROM (
        SELECT rnl.*,
         ...
         WHERE (
           ...
           AND (
                $12 IS NULL OR $12 = '{}'::jsonb
                OR NOT EXISTS (
                  SELECT 1 FROM jsonb_each_text($12) kv
                   WHERE btrim(kv.value) <> ''
                     AND NOT public.rnl_match_col_filter(rnl, sr.recipe_name, kv.key, kv.value)
                )
              )
  ...
  USING p_search, p_research_status, p_content_status, p_category, p_linked,
        p_limit, p_offset, v_active, p_sub_category, p_division, p_drift, v_filters;
