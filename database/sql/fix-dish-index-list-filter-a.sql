-- fix-dish-index-list-filter-a.sql — SMALL: list RPC only (Archived filter fix).
-- Use this if the full list-filter.sql fails with "Failed to fetch (api.supabase.com)".
-- Run after fix-dish-index-ops.sql. Optional: run list-filter-b.sql for Hero Ingredient CSV alias.
-- Or skip entirely and run fix-dish-index-phase-abc-a/b/c.sql instead.
