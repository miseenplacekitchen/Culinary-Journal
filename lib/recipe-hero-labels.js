...
(function (global) {
  ...
  function resolveCssColor(value) {
    ...
    } catch (_) { TcjErr.warn('lib/recipe-hero-labels.js:81', _); }
  ...
  function deriveCaptionColours(accentRgb) {
    var hsl = rgbToHsl(accentRgb);
    return {
      onLight: rgbStr(hslToRgb(hsl.h, Math.max(0.28, hsl.s * 0.95), Math.min(0.30, hsl.l * 0.52))),
      onDark: rgbStr(hslToRgb(hsl.h, Math.max(0.22, hsl.s * 0.52), 0.97))
    };
  }
  ...
  function themeCaptionColors() {
    ...
    return { ring: ring, onLight: derived.onLight, onDark: derived.onDark };
  }
  ...
  function buildHeroSampleCanvas(wrapEl) {
    ...
    try {
      if (img.classList.contains('rp-hero-img')) {
        ...
        ctx.drawImage(
          img,
          (cw / 2) + (crop.panX || 0) - w / 2,
          (ch / 2) + (crop.panY || 0) - h / 2,
          w,
          h
        );
      }
    } catch (_) { TcjErr.warn('lib/recipe-hero-labels.js:207', _); }
    ctx.restore();
    return { ctx: ctx, cw: cw, ch: ch, pxPerVb: cw / (RING_R * 2) };
  }

  function readLuminanceAt(canvasInfo, clockDeg, r) {
    var pt = polarClock(clockDeg, r);
    var lx = Math.max(0, Math.min(canvasInfo.cw - 1, Math.round((pt.x - CX) * canvasInfo.pxPerVb + canvasInfo.cw / 2)));
    var ly = Math.max(0, Math.min(canvasInfo.ch - 1, Math.round((pt.y - CY) * canvasInfo.pxPerVb + canvasInfo.ch / 2)));
    try {
  ...
  var ARC_BUSY_SPREAD = 0.22;
  var ARC_BRIGHT_MEDIAN = 0.50;

  function sampleArcLuminance(canvasInfo, theirAngle, r) {
    if (!canvasInfo) return { median: 0.55, max: 0.55, min: 0.55, spread: 0, samples: [] };
    var clockCenter = toClock(theirAngle);
    var samples = [];
    for (var i = 0; i < ARC_SAMPLE_STOPS.length; i++) {
      var lv = readLuminanceAt(canvasInfo, clockCenter + ARC_SAMPLE_STOPS[i], r);
      if (lv != null) samples.push(lv);
    }
    if (!samples.length) return { median: 0.55, max: 0.55, min: 0.55, spread: 0, samples: [] };
    samples.sort(function (a, b) { return a - b; });
    return {
      min: samples[0],
      median: samples[Math.floor(samples.length / 2)],
      max: samples[samples.length - 1],
      spread: samples[samples.length - 1] - samples[0],
      samples: samples
    };
  }

  function minArcContrast(fgLum, bgSamples) {
    var worst = Infinity;
    for (var i = 0; i < bgSamples.length; i++) {
      worst = Math.min(worst, contrastRatio(fgLum, bgSamples[i]));
    }
    return worst;
  }
  ...
  function pickCaptionColour(colours, canvasInfo, theirAngle, r) {
    ...
    if (!onLightRgb || !onDarkRgb) return colours.onDark;
    var onLightL = lum(onLightRgb);
    var onDarkL = lum(onDarkRgb);

    if (!canvasInfo) return colours.onDark;

    var stats = sampleArcLuminance(canvasInfo, theirAngle, r);
    if (!stats.samples.length) return colours.onDark;

    var darkTextScore = minArcContrast(onLightL, stats.samples);
    var lightTextScore = minArcContrast(onDarkL, stats.samples);

    // Calm pale studio (mint, almonds on grey): dark theme-tinted text.
    if (stats.median >= ARC_BRIGHT_MEDIAN && stats.spread < ARC_BUSY_SPREAD) {
      return colours.onLight;
    }
    // Busy food texture: light theme-tinted text (same rule that makes @username work).
    if (stats.spread >= ARC_BUSY_SPREAD) {
      return colours.onDark;
    }
    return lightTextScore >= darkTextScore ? colours.onDark : colours.onLight;
  }
