...
(function (global) {
  ...
  function tuneForBackground(colour, bgLum, useDarkText) {
    var rgb = parseRgb(colour) || parseRgb(resolveCssColor(colour));
    if (!rgb) return colour;
    if (useDarkText) {
      var deepen = Math.max(0, Math.min(1, (bgLum - 0.40) / 0.38));
      var target = { r: rgb.r * 0.5, g: rgb.g * 0.5, b: rgb.b * 0.5 };
      return rgbStr(mixRgb(rgb, target, deepen * 0.45));
    }
    var lighten = Math.max(0, Math.min(1, (0.50 - bgLum) / 0.38));
    return rgbStr(mixRgb(rgb, { r: 255, g: 255, b: 255 }, lighten * 0.4));
  }

  function pickCaptionColour(colours, stats) {
    var bgLum = stats.median * 0.4 + stats.max * 0.6;
    ...
    var cDark = contrastRatio(lum(onLightRgb), bgLum);
    var cLight = contrastRatio(lum(onDarkRgb), bgLum);
    var useDarkText = cDark >= cLight;
    var base = useDarkText ? colours.onLight : colours.onDark;
    return tuneForBackground(base, bgLum, useDarkText);
  }
  ...
  function paint(wrapEl, opts) {
    ...
    applyCaptionStyle(
      parts.brandText,
      pickCaptionColour(colours, sampleArcLuminance(sampleCanvas, BRAND_ANGLE, brandR))
    );
    ...
    if (handleRaw && parts.handleArc && parts.handleText) {
      ...
      applyCaptionStyle(
        parts.handleText,
        pickCaptionColour(colours, sampleArcLuminance(sampleCanvas, USER_ANGLE, userR))
      );
