 ...
 * • SVG textPath on circular arcs; path direction chosen so text is never inverted.
 */
(function (global) {
  'use strict';
  ...
  /** Arc from fromDeg → toDeg (increasing angle = clockwise on screen). */
  ...
  function paint(wrapEl, opts) {
    ...
    // Brand: arc through 150°, centre via startOffset 50%
    svg.querySelector('#' + id + '-arc-brand').setAttribute('d', arcPath(g, brandLo, brandHi));
    ...
    applyLegibility(brandT, pickColor(sampleLuminance(wrapEl, g, BRAND_CENTER_DEG), themeAccent()));

    // Handle: path 268°→330°; text anchored at 330° (startOffset 100%), grows back along arc
    svg.querySelector('#' + id + '-arc-handle').setAttribute('d', arcPath(g, handleLo, HANDLE_START_DEG));
    var handleT = svg.querySelector('.rh-ring-handle');
    var handle = opts.handle || '';
    if (handle) {
      handleT.style.display = '';
      var handleTp = handleT.querySelector('textPath');
      handleTp.setAttribute('startOffset', '100%');
      handleTp.textContent = handle;
      applyLegibility(handleT, pickColor(sampleLuminance(wrapEl, g, HANDLE_START_DEG), themeAccent()));
    } else {
      handleT.style.display = 'none';
    }
  }
