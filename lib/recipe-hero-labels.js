...
(function (global) {
  ...
  function sampleArcLuminance(canvasInfo, theirAngle, r) {
    if (!canvasInfo) return { median: 0.55, max: 0.55, min: 0.55, spread: 0 };
  ...
  function pickCaptionColour(colours, canvasInfo, theirAngle, r) {
    ...
    // Calm pale studio shots (mint, almonds on grey): dark theme text.
    if (stats.median >= ARC_BRIGHT_MEDIAN && stats.spread < ARC_BUSY_SPREAD) {
      return colours.onLight;
    }
    // Busy food arcs (cauliflower, biryani, chutney): light theme text — same principle as @username.
    if (stats.spread >= ARC_BUSY_SPREAD) {
      return colours.onDark;
    }
    if (stats.median >= ARC_BRIGHT_MEDIAN) {
      return colours.onLight;
    }
    return colours.onDark;
  }
