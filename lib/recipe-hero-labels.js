...
(function (global) {
  ...
  function sampleArcLuminance(canvasInfo, theirAngle, r) {
    if (!canvasInfo) return { median: 0.55, max: 0.55, min: 0.55 };
  ...
  function arcBgLuminance(stats) {
    return stats.median * 0.45 + stats.max * 0.55;
  }

  function pickCaptionLook(colours, stats) {
    var bgLum = arcBgLuminance(stats);
    var darkTextLum = lum(parseRgb(colours.onLight));
    var lightTextLum = lum(parseRgb(colours.onDark));
    var cDarkText = contrastRatio(darkTextLum, bgLum);
    var cLightText = contrastRatio(lightTextLum, bgLum);

    if (cDarkText >= cLightText) {
      return {
        colour: colours.onLight,
        legibility: cDarkText < MIN_CONTRAST ? 'busy-light' : 'none'
      };
    }
    return {
      colour: colours.onDark,
      legibility: cLightText < MIN_CONTRAST ? 'dark-halo' : 'none'
    };
  }
  ...
  function ensureCaptionFilters(ring, haloId, contrastId) {
    var defs = ring.querySelector('defs');
    if (!defs) return;
    if (!ring.querySelector('#' + haloId)) {
      var halo = document.createElementNS('http://www.w3.org/2000/svg', 'filter');
      halo.setAttribute('id', haloId);
      halo.setAttribute('x', '-50%');
      halo.setAttribute('y', '-50%');
      halo.setAttribute('width', '200%');
      halo.setAttribute('height', '200%');
      halo.innerHTML = '<feDropShadow dx="0" dy="0" stdDeviation="0.6" flood-color="#000" flood-opacity="0.35"/>';
      defs.appendChild(halo);
    }
    if (!ring.querySelector('#' + contrastId)) {
      var contrast = document.createElementNS('http://www.w3.org/2000/svg', 'filter');
      contrast.setAttribute('id', contrastId);
      contrast.setAttribute('x', '-40%');
      contrast.setAttribute('y', '-40%');
      contrast.setAttribute('width', '180%');
      contrast.setAttribute('height', '180%');
      contrast.innerHTML =
        '<feDropShadow dx="0" dy="0" stdDeviation="0.4" flood-color="#ffffff" flood-opacity="0.72"/>' +
        '<feDropShadow dx="0" dy="0.4" stdDeviation="0.2" flood-color="#000000" flood-opacity="0.18"/>';
      defs.appendChild(contrast);
    }
  }

  function applyCaptionStyle(textEl, look, haloId, contrastId) {
    textEl.setAttribute('fill', look.colour);
    ...
    var filter = 'none';
    if (look.legibility === 'dark-halo') filter = 'url(#' + haloId + ')';
    else if (look.legibility === 'busy-light') filter = 'url(#' + contrastId + ')';
    textEl.setAttribute('filter', filter);
  ...
  function paint(wrapEl, opts) {
    ...
    var contrastId = parts.haloId.replace('softHalo', 'softContrast');
    ensureCaptionFilters(parts.ring, parts.haloId, contrastId);

    var sampleCanvas = buildHeroSampleCanvas(wrapEl);
    var brandLook = pickCaptionLook(colours, sampleArcLuminance(sampleCanvas, BRAND_ANGLE, brandR));
    applyCaptionStyle(parts.brandText, brandLook, parts.haloId, contrastId);
    ...
    if (handleRaw && parts.handleArc && parts.handleText) {
      ...
      var userLook = pickCaptionLook(colours, sampleArcLuminance(sampleCanvas, USER_ANGLE, userR));
      applyCaptionStyle(parts.handleText, userLook, parts.haloId, contrastId);
