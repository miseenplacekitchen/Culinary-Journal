...
(function (global) {
  ...
  async function renderCareTab(core, selectedSlug) {
    ...
    var selector = '<select class="ap-search" style="max-width:280px;margin-bottom:10px" onchange="gmFocusSpecies(this.value);gmSwitchTab(\'care\');var p=document.getElementById(\'gmpanel-care\');if(p){delete p.dataset.rendered;p.innerHTML=\'\';}gmRenderTab(\'care\')">' +
    ...
    return selector +
      ...
      '<button type="button" class="ing-add-btn" style="font-size:11px;margin-top:8px" onclick="gmExportCareCard(' + JSON.stringify(slug) + ')">Download care-card JSON</button>';
  }
  ...
  async function addCareRow(slug) {
    try {
      ...
      var p = document.getElementById('gmpanel-care');
      if (p) { delete p.dataset.rendered; p.innerHTML = ''; }
      global.gmRenderTab('care');
