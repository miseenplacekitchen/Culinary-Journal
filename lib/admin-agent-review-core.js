...
/** green = auto-approve | yellow = Betty edits | red = auto-reject */
function assessAgentOutcome(structured, row, payload) {
  ...
  const conf = row.import_confidence_score;
  if (conf != null && conf < 40) reasons.push(`Import confidence ${conf}/100`);

  const unknown = parseJsonField(row.unknown_ingredients, []);
  if (Array.isArray(unknown) && unknown.length > 3) {
    reasons.push(`${unknown.length} unknown ingredients`);
  }
  ...
  const hardYellow = reasons.some((r) =>
    r.startsWith('Only ') || r.includes('Missing') || r.includes('unknown ingredients'),
  );
  return {
    outcome: 'review',
    auto_approve: false,
