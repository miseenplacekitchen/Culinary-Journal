...
async function lmFetchProfileIndex(type) {
  try {
    var res = await apiFetch(
      SUPABASE_URL + '/rest/v1/library_profiles?profile_type=eq.' + encodeURIComponent(type) +
      '&select=id,slug,name,status&order=name.asc&limit=1000'
    );
    if (res && res.ok) {
    ...
    console.warn('lm profile index', e);
  }
  return [];
}
...
async function loadLmInterface() {
  ...
  lmPopulateProfileSelect(_lmPendingEditId || '', true);
  ...
  lmFetchProfileIndex(type).then(function (items) {
    _lmProfiles = items;
...
function lmPopulateProfileSelect(selectedId, loading) {
