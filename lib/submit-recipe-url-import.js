...
async function importFromUrl() {
  ...
  if (!sess || !sess.access_token) {
    showImportStatus('Sign in to import from a URL (or paste the recipe text manually).', false);
    if (importBtn) { importBtn.disabled = false; importBtn.textContent = '\u2B07 Import'; }
    return;
  }
  try {
  try {
    var apiRes = await fetch('/api/fetch-recipe-url?url=' + encodeURIComponent(url), {
