...
async function buildSMThemes(container) {
  ...
  try {
    ...
    var disabledLegacy = [];
    try { disabledLegacy = JSON.parse(S.disabled_themes || '[]'); } catch (_) { TcjErr.warn('lib/theme-admin-ui.js:17', _); }
    var catalog = TCJ_mergeThemeCatalog(
      S.theme_catalog,
      disabledLegacy,
      S.seasonal_default_theme || '',
      S.default_theme || ''
    );
    ...
    async function persistCatalog() {
      if (typeof TCJ_syncDefaultThemeFlags === 'function') TCJ_syncDefaultThemeFlags(catalog);
      ...
      await ssSave('default_theme', catalog.default_theme || 'midnight-slate');
    ...
    saveBtn.addEventListener('click', async function () {
      ...
      } catch (e) { TcjErr.warn('lib/theme-admin-ui.js:147', e); }
    });
  ...
  } catch (e) { TcjErr.warn('lib/theme-admin-ui.js:495', e); }
}
