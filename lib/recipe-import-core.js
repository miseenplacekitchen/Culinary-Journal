...
(function (root, factory) {
  ...
  var PARSER_VERSION = '2.3.2-section-preservation';
  ...
  function buildMethodSections(methRegion) {
    ...
    (methRegion || []).forEach(function (ln, idx) {
      ...
      var numMatch = String(ln).match(/^(\d+)[\.\)]\s*(.+)$/);
      if (numMatch) {
        var stepText = numMatch[2].trim();
        if (!stepText || isJunkMethodStep(stepText)) return;
        if (!cur) {
          cur = { name: 'DIRECTIONS', steps: [] };
          sections.push(cur);
        }
        cur.steps.push({ num: numMatch[1], text: stepText });
        flat.push(stepText);
      }
    });
    if (!sections.length && flat.length) {
      sections.push({ name: 'DIRECTIONS', steps: flat.slice() });
    }
  ...
  function segmentRecipeImportText(rawText) {
    ...
    if (method.length < 2) {
      ...
      if (method.length && !methodSections.length) {
        methodSections = [{ name: 'DIRECTIONS', steps: method.slice() }];
      }
    }
