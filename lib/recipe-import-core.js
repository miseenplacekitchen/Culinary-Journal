...
(function (root, factory) {
  ...
  var PARSER_VERSION = '2.3.3-procedure-parsing';
  ...
  function segmentRecipeImportText(rawText) {
    ...
    for (var j = 0; j < lines.length; j++) {
      ...
      if (methHdr < 0 && (
        /^method$/i.test(lines[j]) || /^how\s+to\s+make\b/i.test(lines[j]) ||
        /^instructions?$/i.test(lines[j]) || /^directions?$/i.test(lines[j]) ||
        /^procedure$/i.test(lines[j]) || /^steps?$/i.test(lines[j]) || /^prep\s+work$/i.test(lines[j])
      )) methHdr = j;
    }
