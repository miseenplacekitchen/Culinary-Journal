...
(function() {
  ...
  function markSelectedOption(combo) {
    var hidden = combo.querySelector('.rnl-edit');
    var val = hidden ? hidden.value : '';
    combo.querySelectorAll('.di-combo-opt').forEach(function(btn) {
      btn.classList.toggle('di-combo-selected', btn.dataset.value === val);
    });
  }

  function restoreComboDisplay(combo) {
    ...
    var label = val;
    combo.querySelectorAll('.di-combo-opt').forEach(function(btn) {
      if (btn.dataset.value === val) label = btn.textContent.trim();
    });
    input.value = label;
  }

  function openCombo(combo, input, browseAll) {
    if (combo.classList.contains('di-combo-disabled')) return;
    closeMenus(combo);
    combo.classList.add('open');
    combo.dataset.browsing = browseAll ? '1' : '0';
    if (browseAll) {
      filterComboMenu(combo, '');
      markSelectedOption(combo);
    } else {
    ...
    (root || document).querySelectorAll('.di-combo').forEach(function(combo) {
      ...
      if (input) {
        ...
        input.addEventListener('input', function() {
          combo.dataset.browsing = '0';
          openCombo(combo, input, false);
        });
        input.addEventListener('blur', function() {
          setTimeout(function() {
            if (combo.classList.contains('open') && combo.contains(document.activeElement)) return;
            combo.classList.remove('open');
            restoreComboDisplay(combo);
          }, 120);
        });
      }
      if (menu) {
