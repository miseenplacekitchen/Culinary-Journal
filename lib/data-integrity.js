...
(function (global) {
  ...
  async function loadDataIntegrityPanel(container) {
    ...
    hdr.appendChild(mk('p', "font-family:'DM Sans',sans-serif;font-size:13px;color:var(--text-mid);margin:8px 0 0;line-height:1.6",
      'Automated checks for broken links, duplicate ingredients, and recipe names that do not match the governed database. ' +
      'Run the recipe normaliser to fix spelling across large batches — no manual editing required.'));
    ...
    function renderReport(data) {
      ...
      reportEl.innerHTML =
        ...
        '<div><strong>' + (totals.recipes || 0) + '</strong> recipes</div>' +
    ...
    scanBtn.addEventListener('click', async function () {
      ...
      } catch (e) { TcjErr.warn('admin_data_integrity_report', e); }
