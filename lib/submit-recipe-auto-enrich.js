...
function inferRecipeCategory(name, items, steps, useSteps) {
  ...
  if (typeof TcjCategoryInfer !== 'undefined' && TcjCategoryInfer.inferRecipeCategoryFromBlob) {
    var ingLines = items.map(function (i) { return (i.name || '') + ' ' + (i.note || ''); });
    if (useSteps && steps && steps.length) ingLines.push(steps.join(' '));
    return TcjCategoryInfer.inferRecipeCategoryFromBlob(name, ingLines);
  }
  ...
  var rules = [
    { re: /\b(biriyani|biryani|pilaf|pulao|fried rice)\b/, value: 'The Grain Field' },
    { re: /\b(puttu|idiyappam|idli)\b/, value: 'Breads & Bakery' },
    { re: /\b(roti|rotti|chapati|paratha|naan|flatbread|bread|loaf|roll|pita|kulcha|dosa|appam)\b/, value: 'Breads & Bakery' },
    { re: /\b(cake|cookie|brownie|muffin|cupcake|halwa|ladoo|barfi|kheer|pudding|dessert|sweet|pie|tart)\b/, value: 'Sweet Serenades' },
    { re: /\b(soup|rasam|shorba|broth|stew)\b/, value: 'Pasture & Hoof' },
    { re: /\b(pickle|chutney|jam|preserve|ferment|canning)\b/, value: 'Preserved & Pantry' },
    { re: /\b(salad|raita)\b/, value: 'Garden & Earth' },
    { re: /\b(mocktail|cocktail|martini|smoothie|juice|lassi|chai|tea|coffee|drink|shake|beer|wine|vodka|gin|rum|whiskey|kombucha|horchata|lemonade|tonic|soda|protein shake|energy drink|bubble tea|boba|mocktail|beverage)\b/, value: 'Sips & Stories' },
    { re: /\b(fish|prawn|shrimp|crab|lobster|salmon|tuna|seafood|meen)\b/, value: 'Ocean & River' },
    { re: /\b(chicken|duck|turkey|poultry)\b/, value: 'Feather & Flock' },
    { re: /\b(mutton|lamb|beef|pork|meat|steak|bacon|sausage)\b/, value: 'Pasture & Hoof' },
    { re: /\b(breakfast|pancake|waffle|omelette|porridge)\b/, value: 'Curds, Creams & Eggs' }
  ];
  for (var i = 0; i < rules.length; i++) {
    if (rules[i].re.test(blob)) return rules[i].value;
  }
  if (/\b(rice|dal|lentil|grain)\b/.test(blob)) return 'The Grain Field';
...
function inferDietaryAndHealth(items) {
  ...
  if (/\b(baby|infant|weaning|puree|toddler)\b/i.test(blob)) tags.push('tag-bf');
  if (/\b(recovery|convalescen|postpartum|nourish|healing|gentle|sick.?day)\b/i.test(blob)) tags.push('tag-recovery');
  return tags;
}
...
function inferMealOccasionStyleTags(name, category, items, times) {
  ...
  var norm = (typeof normalizeRecipeCategory === 'function') ? normalizeRecipeCategory(category) : category;
  if (typeof TcjCategoryInfer !== 'undefined' && TcjCategoryInfer.inferUserFriendlyTags) {
    var ingLines = (items || []).map(function (i) { return (i.name || '') + ' ' + (i.note || ''); });
    var uf = TcjCategoryInfer.inferUserFriendlyTags(name, ingLines, category);
    uf.tagIds.forEach(function (id) { if (tags.indexOf(id) < 0) tags.push(id); });
  }
  ...
  else if (norm === 'Sweet Serenades' || category === 'Sweet Serenades') tags.push('tag-mt-dessert');
  ...
  else if (/\b(breakfast|brunch|pancake|waffle|omelette|porridge|idli|dosa)\b/.test(blob)
      || category === 'Rise & Shine' || norm === 'Curds, Creams & Eggs') tags.push('tag-mt-breakfast');
  else if (norm === 'Feather & Flock' || norm === 'Pasture & Hoof' || norm === 'Ocean & River'
      || category === 'Meat & Fire' || category === 'Ocean & River') tags.push('tag-mt-main');
  else if (norm === 'Breads & Bakery' || category === 'Breads & Bakes') tags.push('tag-mt-bread');
  else tags.push('tag-mt-side');

  if ((times.prep + times.cook) <= 30) tags.push('tag-quick');
  if (/\b(comfort|home|simple)\b/.test(blob) || norm === 'The Grain Field' || category === 'Grains & Comfort') tags.push('tag-comfort');
...
function expandLevelsTagsSection() {
  if (typeof ensureLevelsTagsVisible === 'function') {
    ensureLevelsTagsVisible();
    return;
  }
  var panel = document.getElementById('panel-levels') || document.getElementById('subsection-levels-tags');
  if (!panel) return;
  panel.classList.remove('collapsed');
  var head = panel.querySelector(':scope > .sr-panel-head');
  if (head) head.setAttribute('aria-expanded', 'true');
  panel.querySelectorAll('.sr-panel-nested').forEach(function(n) {
    n.classList.remove('collapsed');
    var nh = n.querySelector(':scope > .sr-panel-head');
