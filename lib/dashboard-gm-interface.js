...
function reloadGmInterface() {
  ...
  if (root) {
    ...
    delete root.dataset.ifShell;
    root._ifShell = null;
    root.innerHTML = '';
  }
  _gmCache = {};
  if (typeof window !== 'undefined') window._gmLookups = null;
  loadGmInterface(true);
}

function gmGetSectionPanel(tab) {
  var root = document.getElementById('gm-interface-content');
  if (root && root._ifShell && root._ifShell.panels) {
    return root._ifShell.panels[tab];
  }
  return document.getElementById('gmpanel-' + tab);
}

function gmRefreshGmTab(tab) {
  var root = document.getElementById('gm-interface-content');
  if (root && root._ifShell) {
    root._ifShell.invalidate(tab);
    root._ifShell.activate(tab);
    return;
  }
  var p = document.getElementById('gmpanel-' + tab);
  if (p) { p.dataset.rendered = ''; p.innerHTML = ''; }
  gmRenderTab(tab);
}
...
async function gmLoadCoreData() {
  ...
  var core = {
    ...
    pages: await gmFetchSafe('/rest/v1/site_pages?or=(path.eq.garden-directory.html,path.eq.garden-plant.html,path.eq.my-garden.html,path.eq.garden-journal.html)&select=path,name,visibility') || [],
    ...
    varieties: await gmFetchSafe('/rest/v1/plant_varieties?select=id,slug,name,lineage_type,is_published,plant_id&order=name') || [],
    imports: await gmFetchSafe('/rest/v1/garden_import_queue?select=id,source_path,species_name,species_slug,status,variety_count,payload,created_at&order=created_at.desc&limit=250') || []
  };
  _gmCache.core = core;
  return core;
}
...
function gmFocusSpecies(slug) {
  ...
  var sp = document.getElementById('gmpanel-species');
  if (sp) { delete sp.dataset.rendered; sp.innerHTML = ''; }
  reloadGmInterface();
}

async function gmSaveSpeciesFields(slug, btn) {
  if (!slug) return;
  var fields = {};
  ['care_summary', 'botanical_name', 'plant_family', 'origin', 'size_height', 'size_spread', 'culinary_applications'].forEach(function (k) {
    var el = document.getElementById('gm-edit-' + k);
    if (el) fields[k] = el.value;
  });
  if (btn) { btn.disabled = true; btn.textContent = 'Saving…'; }
  try {
    await rpc('admin_patch_garden_species', { p_slug: slug, p_fields: fields });
    delete _gmCache['species:' + slug];
    delete _gmCache.core;
    if (btn) btn.textContent = 'Saved';
    setTimeout(function () { if (btn) { btn.disabled = false; btn.textContent = 'Save species fields'; } }, 1200);
  } catch (e) {
    alert('Save failed: ' + e.message + '\nRun garden-v4-09-import-rpcs.sql if RPC is missing.');
    if (btn) { btn.disabled = false; btn.textContent = 'Save species fields'; }
  }
}

async function gmApplyRefreshedImports(btn) {
  var core = await gmLoadCoreData();
  var pending = (core.imports || []).filter(function (r) {
    return _gmRefreshedImportSlugs.indexOf(r.species_slug) >= 0 &&
      r.payload && r.payload.varieties && r.payload.varieties.length &&
      r.status !== 'approved';
  });
  if (!pending.length) {
    alert('No pending refreshed imports — all 12 payloads may already be applied.');
    return;
  }
  if (!confirm('Apply ' + pending.length + ' refreshed import payload(s) from phase54?')) return;
  if (btn) { btn.disabled = true; btn.textContent = 'Applying…'; }
  var ok = 0;
  var fail = 0;
  for (var i = 0; i < pending.length; i++) {
    try {
      await rpc('admin_apply_garden_import', { p_queue_id: pending[i].id });
      ok++;
    } catch (e) {
      fail++;
      console.warn('apply refreshed', pending[i].species_slug, e);
    }
  }
  delete _gmCache.core;
  reloadGmInterface();
  alert('Refreshed imports: ' + ok + ' applied' + (fail ? ', ' + fail + ' failed' : '') + '.');
  if (btn) { btn.disabled = false; btn.textContent = 'Apply refreshed 12 imports'; }
}

async function gmPublishKitchenBatch(btn) {
  var core = await gmLoadCoreData();
  var kitchen = (core.plants || []).filter(function (p) {
    return _gmKitchenSlugs.indexOf(p.slug) >= 0 && !p.is_published;
  });
  if (!kitchen.length) {
    alert('All kitchen-priority species are already published (or shells missing).');
    return;
  }
  if (!confirm('Publish ' + kitchen.length + ' kitchen species on the public directory?\n\n' +
    kitchen.map(function (p) { return '• ' + p.common_name; }).join('\n'))) return;
  if (btn) { btn.disabled = true; btn.textContent = 'Publishing…'; }
  var ok = 0;
  for (var i = 0; i < kitchen.length; i++) {
    try {
      var res = await supaFetch('/rest/v1/plants?slug=eq.' + encodeURIComponent(kitchen[i].slug), {
        method: 'PATCH',
        headers: { Prefer: 'return=minimal' },
        body: JSON.stringify({ is_published: true })
      });
      if (res.ok) ok++;
    } catch (e) { console.warn('publish kitchen', kitchen[i].slug, e); }
  }
  delete _gmCache.core;
  if (typeof loadGardenMgmtStats === 'function') loadGardenMgmtStats();
  reloadGmInterface();
  alert('Published ' + ok + ' kitchen species.');
  if (btn) { btn.disabled = false; btn.textContent = 'Publish all draft kitchen (15)'; }
}

async function gmBuildKitchenGoLivePanel(core) {
  var plants = (core.plants || []).filter(function (p) {
    return _gmKitchenSlugs.indexOf(p.slug) >= 0;
  });
  if (!plants.length) {
    return '<p style="font-size:13px;color:var(--warn,#c97)">Kitchen species shells not found — run RUN-GARDEN-GO-LIVE.sql.</p>';
  }
  var ids = plants.map(function (p) { return p.id; }).filter(Boolean);
  var careRows = ids.length
    ? (await gmFetchSafe('/rest/v1/plant_climate_care?plant_id=in.(' + ids.join(',') + ')&select=plant_id') || [])
    : [];
  var careByPlant = {};
  careRows.forEach(function (c) {
    careByPlant[c.plant_id] = (careByPlant[c.plant_id] || 0) + 1;
  });
  var varByPlant = {};
  (core.varieties || []).forEach(function (v) {
    varByPlant[v.plant_id] = (varByPlant[v.plant_id] || 0) + 1;
  });
  var refreshedPending = (core.imports || []).filter(function (r) {
    return _gmRefreshedImportSlugs.indexOf(r.species_slug) >= 0 &&
      r.payload && r.payload.varieties && r.payload.varieties.length &&
      r.status !== 'approved';
  }).length;
  ...
  var rows = plants.map(function (p) {
    var careN = careByPlant[p.id] || 0;
    var varN = varByPlant[p.id] || 0;
    var profileOk = careN >= 3;
    return '<tr><td><strong>' + gmEsc(p.common_name) + '</strong><br><span style="font-family:monospace;font-size:10px">' + gmEsc(p.slug) + '</span></td>' +
      '<td>' + (p.is_published ? '<span style="color:#6dc86d">Live</span>' : '<span style="color:var(--warn,#c97)">Draft</span>') + '</td>' +
      '<td>' + careN + ' care · ' + varN + ' cultivars</td>' +
      '<td>' + (profileOk ? '✓ profile' : '—') + '</td>' +
      '<td style="white-space:nowrap">' +
      '<a href="garden-plant.html?slug=' + encodeURIComponent(p.slug) + '" target="_blank" rel="noopener" style="font-size:11px;color:var(--accent);margin-right:6px">Preview ↗</a>' +
      '<button type="button" class="ing-add-btn" style="font-size:10px;padding:3px 8px" onclick="gmTogglePublish(' + JSON.stringify(p.slug) + ',' + (p.is_published ? 'true' : 'false') + ',this)">' +
      (p.is_published ? 'Unpublish' : 'Publish') + '</button></td></tr>';
  }).join('');

  var draftN = plants.filter(function (p) { return !p.is_published; }).length;

  return '<p style="font-family:DM Sans,sans-serif;font-size:13px;color:var(--text-mid);line-height:1.6;margin:0 0 12px">' +
    'Phase 54 seeded care + calendar for these 15 kitchen species. Apply refreshed cultivar imports, review previews, then publish.</p>' +
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:14px">' +
    (refreshedPending
      ? '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 14px" onclick="gmApplyRefreshedImports(this)">Apply refreshed imports (' + refreshedPending + ')</button>'
      : '<span style="font-size:12px;color:#6dc86d">Refreshed imports applied ✓</span>') +
    (draftN
      ? '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 14px" onclick="gmPublishKitchenBatch(this)">Publish all draft kitchen (' + draftN + ')</button>'
      : '') +
    '</div>' +
    gmTableWrap('<tr><th>Species</th><th>Status</th><th>Data</th><th>Profile</th><th></th></tr>', rows);
}

async function gmApplyImport(queueId, btn) {
  if (!queueId || !confirm('Apply this import payload to the database? Existing cultivars with the same slug will be updated.')) return;
  if (btn) { btn.disabled = true; btn.textContent = 'Applying…'; }
  try {
    var res = await rpc('admin_apply_garden_import', { p_queue_id: queueId });
    alert('Import applied: ' + (res && res.varieties_upserted != null ? res.varieties_upserted + ' cultivars' : 'done'));
    delete _gmCache.core;
    reloadGmInterface();
  } catch (e) {
...
async function gmApplyAllPendingImports(btn) {
  var core = await gmLoadCoreData();
  var pending = (core.imports || []).filter(function (r) {
    return r.payload && r.payload.varieties && r.payload.varieties.length && r.status !== 'approved';
  });
  if (!pending.length) {
    alert('No pending import rows with payloads.');
    return;
  }
  if (!confirm('Apply ' + pending.length + ' import payload(s)? Cultivars upsert for every species with a shell row. Draft species stay off the public directory until you publish them.')) return;
  if (btn) { btn.disabled = true; btn.textContent = 'Applying…'; }
  try {
    var res = await rpc('admin_apply_all_garden_imports', {});
    delete _gmCache.core;
    reloadGmInterface();
    var msg = 'Batch apply finished.\n\nApplied: ' + (res && res.applied != null ? res.applied : '?') +
      '\nFailed: ' + (res && res.failed != null ? res.failed : '?');
    if (res && res.errors && res.errors.length) {
      msg += '\n\nFirst errors:\n' + res.errors.slice(0, 8).map(function (e) {
        return (e.species_slug || e.species_name || '?') + ': ' + (e.error || '');
      }).join('\n');
    }
    alert(msg);
  } catch (e) {
    ...
    var errors = [];
    for (var i = 0; i < pending.length; i++) {
      if (btn) btn.textContent = 'Applying ' + (i + 1) + '/' + pending.length + '…';
      try {
        await rpc('admin_apply_garden_import', { p_queue_id: pending[i].id });
        ok++;
      } catch (err) {
        fail++;
        errors.push((pending[i].species_slug || pending[i].species_name) + ': ' + err.message);
      }
    }
    delete _gmCache.core;
    reloadGmInterface();
    alert('Batch apply finished (fallback loop).\n\nApplied: ' + ok + '\nFailed: ' + fail +
      (errors.length ? '\n\nFirst errors:\n' + errors.slice(0, 8).join('\n') : ''));
  }
  if (btn) { btn.disabled = false; btn.textContent = 'Apply all pending imports (' + pending.length + ')'; }
}

async function gmAttachImportPayload(queueId, fileInput) {
  if (!queueId || !fileInput || !fileInput.files || !fileInput.files[0]) return;
  var text = await fileInput.files[0].text();
  var payload = JSON.parse(text);
  try {
    var res = await supaFetch('/rest/v1/garden_import_queue?id=eq.' + encodeURIComponent(queueId), {
      method: 'PATCH',
      headers: { Prefer: 'return=minimal' },
      body: JSON.stringify({
        payload: payload,
        variety_count: payload.variety_count || (payload.varieties ? payload.varieties.length : 0),
        status: 'parsed'
      })
    });
    if (!res.ok) throw new Error(String(res.status));
    ...
    alert('Payload upload failed: ' + e.message);
  }
}

async function gmRenderTabIntoPanel(tab, panel, core) {
  if (!panel) return;
  ...
  try {
    if (tab === 'refdata') {
      panel.innerHTML = typeof GmRefData !== 'undefined' ? GmRefData.render() : '<p>Load dashboard-gm-refdata.js</p>';
      if (typeof GmRefData !== 'undefined') GmRefData.init(core);
    }
    else if (tab === 'species' && typeof GmEditors !== 'undefined') panel.innerHTML = await GmEditors.renderSpeciesTab(core, _gmSelectedSlug);
    else if (tab === 'species') panel.innerHTML = await gmBuildSpeciesTab(core);
    else if (tab === 'varieties' && typeof GmEditors !== 'undefined') panel.innerHTML = await GmEditors.renderVarietiesTab(core);
    else if (tab === 'varieties') panel.innerHTML = await gmBuildVarietiesTab(core);
    else if (tab === 'care' && typeof GmEditors !== 'undefined') panel.innerHTML = await GmEditors.renderCareTab(core, _gmSelectedSlug);
    else if (tab === 'care') panel.innerHTML = await gmBuildCareTab(core);
    else if (tab === 'guilds' && typeof GmGuilds !== 'undefined') panel.innerHTML = await GmGuilds.renderGuildsTab(core);
    else if (tab === 'guilds') panel.innerHTML = '<p>Load dashboard-gm-guilds.js</p>';
    else if (tab === 'kitchen') panel.innerHTML = await gmBuildKitchenTab(core);
    else if (tab === 'health') panel.innerHTML = (await gmBuildKitchenGoLivePanel(core)) + gmBuildHealthTab(core);
  } catch (e) {
...
async function loadGmInterface(force) {
  var root = document.getElementById('gm-interface-content');
  if (!root || typeof AdminTabNav === 'undefined') {
    if (root && force) root.innerHTML = '<p style="font-family:DM Sans,sans-serif;font-size:13px;color:var(--text-mid)">Admin tab navigation failed to load.</p>';
    return;
  }
  ...
  if (force) {
    ...
    _gmCache = {};
    if (typeof window !== 'undefined') window._gmLookups = null;
  } else if (root.dataset.ifShell === '1' && root._ifShell) {
    var active = localStorage.getItem('tcj_active_gm_tab') || 'hub';
    root._ifShell.activate(active);
    return;
  }
  ...
  var TAB_DEFS = [
    { key: 'refdata', label: 'Lookup data', group: 'Reference' },
    { key: 'species', label: 'Species', group: 'Editors' },
    { key: 'varieties', label: 'Cultivars', group: 'Editors' },
    { key: 'care', label: 'Care & calendar', group: 'Editors' },
    { key: 'guilds', label: 'Guilds', group: 'Editors' },
    { key: 'kitchen', label: 'Kitchen links', group: 'Editors' },
    { key: 'health', label: 'Import & health', group: 'Operations' }
  ];
  ...
  var shellHost = document.createElement('div');
  root.appendChild(shellHost);

  var sections = [
    AdminTabNav.hubSection({
      group: 'Garden',
      subtitle: 'Species, cultivars, and import queue',
      loadHub: function (panel, ctx) {
        return gmLoadCoreData().then(function (c) {
          var pendingImports = (c.imports || []).filter(function (i) { return i.status === 'pending'; }).length;
          return {
            stats: [
              { num: (c.plants || []).length, label: 'Species' },
              { num: (c.varieties || []).length, label: 'Cultivars' },
              { num: pendingImports, label: 'Import queue' }
            ],
            actions: [
              { label: 'Import & health', desc: 'Queue and go-live checks', onClick: function () { ctx.activate('health'); } },
              { label: 'Species editor', desc: 'Plant records', onClick: function () { ctx.activate('species'); } },
              { label: 'Care & calendar', desc: 'Climate care cards', onClick: function () { ctx.activate('care'); } },
              { label: 'Lookup data', desc: 'Categories and ratings', onClick: function () { ctx.activate('refdata'); } },
              { label: 'Refresh GM data', desc: 'Reload from Supabase', onClick: function () { reloadGmInterface(); } }
            ]
          };
        });
      }
  ...
  TAB_DEFS.forEach(function (td) {
    sections.push({
      ...
      render: function (panel) {
        return gmRenderTabIntoPanel(td.key, panel, core);
      }
    });
  });

  var defaultKey = localStorage.getItem('tcj_active_gm_tab') || 'hub';
  var validKeys = ['hub'].concat(TAB_DEFS.map(function (d) { return d.key; }));
  if (validKeys.indexOf(defaultKey) < 0) defaultKey = 'hub';

  AdminTabNav.buildInterfaceShell(shellHost, {
    storageKey: 'tcj_active_gm_tab',
    defaultKey: defaultKey,
    banner: false,
    shared: { core: core },
    sections: sections
  });
  root._ifShell = shellHost._ifShell;
  root.dataset.ifShell = '1';
...
function gmSwitchTab(tab) {
  ...
  ['refdata', 'species', 'varieties', 'care', 'guilds', 'kitchen', 'health'].forEach(function (k) {
...
async function gmRenderTab(tab, core) {
  var panel = gmGetSectionPanel(tab);
...
async function gmBuildRefDataTab(core) {
  var blocks = gmAccordion('Display policy',
    ...
    'Excel export (completed records) and care-card PPT download from Care tab (phase57).</p>', true);
...
async function gmBuildSpeciesTab(core) {
  ...
  if (_gmSelectedSlug) {
    ...
    if (detail && detail.plant) {
      ...
      detailHtml += '<p style="font-size:11px;color:var(--text-muted);margin:12px 0 6px">Editable species fields (public names + care summary)</p>' +
        '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));gap:10px;font-family:DM Sans,sans-serif;font-size:12px">' +
        [
          ['botanical_name', 'Botanical name', p.botanical_name],
          ['plant_family', 'Family', p.plant_family],
          ['origin', 'Origin', p.origin],
          ['size_height', 'Height', p.size_height],
          ['size_spread', 'Spread', p.size_spread]
        ].map(function (row) {
          return '<label><span style="color:var(--text-muted);font-size:10px;text-transform:uppercase">' + gmEsc(row[1]) + '</span>' +
            '<input id="gm-edit-' + row[0] + '" class="ap-search" style="margin-top:4px;width:100%" value="' + gmEsc(row[2] || '') + '"></label>';
        }).join('') +
        '<label style="grid-column:1/-1"><span style="color:var(--text-muted);font-size:10px;text-transform:uppercase">Care summary (public)</span>' +
        '<textarea id="gm-edit-care_summary" class="ap-search" rows="3" style="margin-top:4px;width:100%">' + gmEsc(p.care_summary || '') + '</textarea></label>' +
        '<label style="grid-column:1/-1"><span style="color:var(--text-muted);font-size:10px;text-transform:uppercase">Culinary applications</span>' +
        '<textarea id="gm-edit-culinary_applications" class="ap-search" rows="2" style="margin-top:4px;width:100%">' + gmEsc(p.culinary_applications || '') + '</textarea></label>' +
        '</div>' +
        '<button type="button" class="ing-add-btn" style="margin-top:10px;font-size:11px;padding:6px 14px" onclick="gmSaveSpeciesFields(' + JSON.stringify(p.slug) + ',this)">Save species fields</button>' +
        '<p style="font-size:11px;color:var(--text-muted);margin:12px 0 0">Full 83-field profile + cultivar layer via import queue Apply.</p></div>';
    }
  }
...
async function gmBuildCareTab(core) {
  ...
  var selector = '<select class="ap-search" style="max-width:320px;margin-bottom:12px" onchange="gmFocusSpecies(this.value);gmSwitchTab(\'care\');gmRefreshGmTab(\'care\')">' +
    (core.plants || []).map(function (p) {
      return '<option value="' + gmEsc(p.slug) + '"' + (p.slug === slug ? ' selected' : '') + '>' + gmEsc(p.common_name) + '</option>';
    }).join('') + '</select>';

  return '<p style="font-family:DM Sans,sans-serif;font-size:13px;color:var(--text-mid);margin:0 0 8px">Care card fields per climate. Export JSON or PPT for print pipeline.</p>' +
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:12px;align-items:center">' +
    selector +
    '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 12px" onclick="gmExportCareCard(' + JSON.stringify(slug) + ')">Download care-card JSON</button>' +
    '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 12px" onclick="gmExportCareCardPptx(' + JSON.stringify(slug) + ')">Download care-card PPT</button>' +
    '</div>' +
...
async function gmBuildVarietiesTab(core) {
  var vars = core.varieties || [];
  var plantMap = {};
  (core.plants || []).forEach(function (p) { plantMap[p.id] = p; });
  var rows = vars.map(function (v) {
    var sp = plantMap[v.plant_id] || {};
    return '<tr><td>' + gmEsc(sp.common_name || '—') + '</td>' +
      '<td style="font-family:monospace;font-size:11px">' + gmEsc(v.slug) + '</td>' +
      '<td><strong>' + gmEsc(v.name) + '</strong></td>' +
      '<td>' + gmEsc(v.lineage_type || '—') + '</td>' +
      '<td>' + (v.is_published ? 'Yes' : 'No') + '</td>' +
      '<td><a href="garden-plant.html?slug=' + encodeURIComponent(sp.slug || '') + '&variety=' + encodeURIComponent(v.slug || '') + '" target="_blank" rel="noopener" style="font-size:11px;color:var(--accent)">Preview ↗</a></td></tr>';
  }).join('') || '<tr><td colspan="6" class="ap-empty-row">No cultivars — run RUN-GARDEN-V4.sql</td></tr>';

  var pub = vars.filter(function (v) { return v.is_published; }).length;
  return gmAccordion('Cultivar registry (' + pub + ' published / ' + vars.length + ' total)',
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin:0 0 12px">' +
    '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 12px" onclick="gmExportCultivarsCsv()">Export cultivars CSV (TCJ)</button>' +
    '</div>' +
    '<p style="font-size:13px;color:var(--text-mid);margin:0 0 12px">Public site shows <strong>names + lineage labels</strong> only. Slugs used in URLs for routing.</p>' +
    gmTableWrap('<tr><th>Species</th><th>Slug (admin)</th><th>Name (public)</th><th>Lineage</th><th>Published</th><th></th></tr>', rows), true) +
    gmAccordion('Per-variety kitchen hinges',
    '<p style="font-size:12px;color:var(--text-mid)">Each cultivar can map to a governed ingredient via <code>variety_ingredients</code>. Species-level hinge used as fallback until distinct ingredients exist.</p>', false);
}
...
function gmBuildHealthTab(core) {
  ...
  (core.plants || []).forEach(function (p) {
    if (p.is_published && !(core.hinges || []).some(function (h) { return h.plant_id === p.id; })) {
      health.push('Published species missing kitchen hinge: ' + p.common_name);
    }
  });
  if (!(core.varieties || []).length) health.push('No cultivars — run RUN-GARDEN-V4.sql');
  if ((core.varieties || []).length < 10) health.push('Only ' + (core.varieties || []).length + ' cultivars — ingest more via docx pipeline.');
  var draft = (core.plants || []).filter(function (p) { return !p.is_published; });
  if (draft.length) health.push(draft.length + ' draft species (curate in GM, then Publish when ready).');
  ...
  var healthHtml = health.length
    ...
    : '<p style="color:#6dc86d;font-size:13px;margin:0">No blocking issues detected for Garden v4 slice.</p>';
  ...
  return gmAccordion('Health checks', healthHtml, true) +
    ...
    gmAccordion('Content counts',
      ...
      '<div><strong>' + (core.varieties || []).length + '</strong> cultivars</div>' +
      '<div><strong>' + (core.topics || []).length + '</strong> topics</div>' +
      '</div>', false) +
    gmAccordion('Import queue',
      (function () {
        var imp = core.imports || [];
        if (!imp.length) {
          return '<p style="font-size:13px;color:var(--text-mid)">No import rows — run <code>scripts/ingest-garden-variety-assessment.py --all</code>, then <code>scan-garden-inbox-queue.py</code> + <code>garden-v4-10-batch-import-queue.sql</code>.</p>';
        }
        var pendingCount = imp.filter(function (r) {
          return r.payload && r.payload.varieties && r.payload.varieties.length && r.status !== 'approved';
        }).length;
        var toolbar = pendingCount
          ? '<div style="margin-bottom:12px;display:flex;flex-wrap:wrap;gap:8px;align-items:center">' +
            '<button type="button" class="ing-add-btn" id="gm-apply-all-btn" onclick="gmApplyAllPendingImports(this)">Apply all pending imports (' + pendingCount + ')</button>' +
            '<span style="font-size:11px;color:var(--text-mid)">Requires species shells — run <code>RUN-GARDEN-GO-LIVE.sql</code> on Supabase first. After <code>fix-phase54-import-payload-refresh.sql</code>, click Apply to load re-parsed cultivars.</span></div>'
          : '';
        return toolbar + gmTableWrap('<tr><th>Species</th><th>Status</th><th>Count</th><th>Payload</th><th></th></tr>',
          imp.map(function (r) {
            var hasPayload = r.payload && r.payload.varieties && r.payload.varieties.length;
            var actions = '';
            if (hasPayload && r.status !== 'approved') {
              actions += '<button type="button" class="ing-add-btn" style="font-size:10px;padding:3px 8px;margin-right:4px" onclick="gmApplyImport(' + JSON.stringify(r.id) + ',this)">Apply</button>';
            }
            if (!hasPayload && r.status !== 'approved') {
              actions += '<label class="ing-add-btn" style="font-size:10px;padding:3px 8px;cursor:pointer;display:inline-block">' +
                'JSON<input type="file" accept=".json,application/json" style="display:none" onchange="gmAttachImportPayload(' + JSON.stringify(r.id) + ',this)"></label>';
            }
            return '<tr><td><strong>' + gmEsc(r.species_name) + '</strong><br><span style="font-family:monospace;font-size:10px">' + gmEsc(r.species_slug || '') + '</span></td>' +
              '<td>' + gmEsc(r.status) + '</td><td>' + (r.variety_count || 0) + '</td>' +
              '<td style="font-size:11px">' + (hasPayload ? hasPayload + ' in DB' : '— attach JSON') + '</td>' +
              '<td style="white-space:nowrap">' + actions + '</td></tr>';
          }).join(''));
      })(), false) +
    gmAccordion('Export pipeline',
      '<p style="font-size:13px;color:var(--text-mid);line-height:1.7;margin:0 0 10px">' +
      '<strong>CSV:</strong> Cultivars tab → Export cultivars CSV.<br>' +
      '<strong>Care card JSON / PPT:</strong> Care tab → Download care-card JSON or PPT.<br>' +
      '<strong>Kitchen profiles:</strong> phase54 SQL ✅ — use Kitchen go-live panel above.<br>' +
      '<strong>Excel (83-field):</strong> phase56 SQL ✅ — regenerate via scripts/generate-garden-excel-profiles.py.<br>' +
      '<strong>Guilds + media:</strong> Guilds tab + species media upload (phase57 SQL).</p>' +
      '<button type="button" class="ing-add-btn" style="font-size:11px;padding:6px 12px;margin-top:4px" onclick="gmExportCultivarsCsv()">Export all cultivars CSV</button>', false);
}

async function gmExportCultivarsCsv() {
  var core = await gmLoadCoreData();
  if (typeof GardenExport === 'undefined') {
    alert('garden-export.js not loaded');
    return;
  }
  GardenExport.exportCultivarsCsv(core.varieties || [], core.plants || []);
}

async function gmExportCareCard(slug) {
  if (!slug) return;
  var core = await gmLoadCoreData();
  var detail = await gmLoadSpeciesDetail(slug);
  if (!detail || typeof GardenExport === 'undefined') {
    alert('Could not export — load garden-export.js');
    return;
  }
  var climate = (core.climates || []).find(function (c) { return c.slug === 'humid-subtropical'; });
  GardenExport.exportCareCardJson(detail, core, climate ? climate.slug : null, slug);
}

async function gmExportCareCardPptx(slug) {
  if (!slug) return;
  var core = await gmLoadCoreData();
  var detail = await gmLoadSpeciesDetail(slug);
  if (!detail || typeof GardenExport === 'undefined') {
  ...
  var climate = (core.climates || []).find(function (c) { return c.slug === 'humid-subtropical'; });
  try {
    await GardenExport.exportCareCardPptx(detail, core, climate ? climate.slug : null, slug);
  } catch (e) {
    alert(e.message || e);
  }
}

window.gmExportCareCard = gmExportCareCard;
window.gmExportCareCardPptx = gmExportCareCardPptx;
window.gmApplyImport = gmApplyImport;
window.gmApplyAllPendingImports = gmApplyAllPendingImports;
window.gmApplyRefreshedImports = gmApplyRefreshedImports;
window.gmPublishKitchenBatch = gmPublishKitchenBatch;
window.gmAttachImportPayload = gmAttachImportPayload;
window.gmFetch = gmFetch;
window.gmFetchSafe = gmFetchSafe;
window.gmTableWrap = gmTableWrap;
window.gmAccordion = gmAccordion;
window.gmLoadSpeciesDetail = gmLoadSpeciesDetail;
window.gmSwitchTab = gmSwitchTab;
window.gmRefreshGmTab = gmRefreshGmTab;
window.gmRenderTab = gmRenderTab;
window._gmCache = _gmCache;
