...
async function gmLoadCoreData() {
  ...
  var core = {
    ...
    imports: await gmFetchSafe('/rest/v1/garden_import_queue?select=source_path,species_name,status,variety_count,created_at&order=created_at.desc&limit=20') || []
  };
  _gmCache.core = core;
  return core;
}
  ...
  return gmAccordion('Health checks', healthHtml, true) +
    ...
    gmAccordion('Import queue',
      (function () {
        ...
        if (!imp.length) return '<p style="font-size:13px;color:var(--text-mid)">No import rows — run <code>RUN-GARDEN-V4.sql</code> or <code>scripts/ingest-garden-variety-assessment.py</code>.</p>';
        return gmTableWrap('<tr><th>Source</th><th>Species</th><th>Status</th><th>Count</th></tr>',
          imp.map(function (r) {
            return '<tr><td style="font-size:11px">' + gmEsc(r.source_path) + '</td><td>' + gmEsc(r.species_name) + '</td><td>' + gmEsc(r.status) + '</td><td>' + (r.variety_count || 0) + '</td></tr>';
          }).join(''));
      })(), false) +
    gmAccordion('Export pipeline (planned)',
      '<p style="font-size:13px;color:var(--text-mid);line-height:1.7;margin:0">' +
      '<strong>Excel:</strong> completed records in TCJ column format.<br>' +
      '<strong>PPT:</strong> care cards per species (Artichoke template).</p>', false);
}
