...
(function (global) {
  ...
  async function fetchIndex(force) {
    ...
    if (!url || !key) {
      if (global.TcjErr) TcjErr.warn('ingredient-lookup:config', 'missing Supabase URL/key');
    ...
    try {
      ...
      if (!res.ok) {
        if (global.TcjErr && !_cache) {
          TcjErr.warn('ingredient-lookup:http', res.status + ' ' + res.statusText);
          TcjErr.bannerOnce('tcj-ing-lookup-fail', 'Ingredient directory could not load — autocomplete may be limited.');
        }
        return _cache || buildIndex([]);
      }
      ...
      if (global.TcjErr) {
        TcjErr.warn('ingredient-lookup:fetch', e);
        if (!_cache) TcjErr.bannerOnce('tcj-ing-lookup-fail', 'Ingredient directory could not load — autocomplete may be limited.');
      }
      return _cache || buildIndex([]);
    }
  }
