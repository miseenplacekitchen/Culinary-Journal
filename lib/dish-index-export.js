...
(function() {
  ...
  async function mergeTemplateXmlFooters(templateBuf, outputBuf, sheetPaths, tableRefPatch) {
    if (!window.JSZip) return outputBuf;
    var tplZip = await window.JSZip.loadAsync(templateBuf);
    var outZip = await window.JSZip.loadAsync(outputBuf);

    function mergeSheetFooter(outputXml, templateXml) {
      var closeTag = '</sheetData>';
      var outIdx = outputXml.indexOf(closeTag);
      var tplIdx = templateXml.indexOf(closeTag);
      if (outIdx < 0 || tplIdx < 0) return outputXml;
      return outputXml.substring(0, outIdx + closeTag.length) + templateXml.substring(tplIdx + closeTag.length);
    }

    var path;
    for (path of sheetPaths) {
      var tplFile = tplZip.file(path);
      var outFile = outZip.file(path);
      if (!tplFile || !outFile) continue;
      var tplXml = await tplFile.async('string');
      var outXml = await outFile.async('string');
      outZip.file(path, mergeSheetFooter(outXml, tplXml));
    }

    if (tableRefPatch) {
      var tableFile = outZip.file(tableRefPatch.path);
      if (tableFile) {
        var tableXml = await tableFile.async('string');
        tableXml = tableXml.replace(/ref="B2:BB\d+"/, 'ref="' + tableRefPatch.ref + '"');
        outZip.file(tableRefPatch.path, tableXml);
      }
    }
  ...
  async function buildWorkbookFromTemplate(opts) {
    ...
    var lastRow = Math.max(START_ROW + 1, START_ROW + rows.length);
    out = await mergeTemplateXmlFooters(buf, out, ['xl/worksheets/sheet2.xml'], {
      path: 'xl/tables/table23.xml',
      ref: 'B2:BB' + lastRow
    });
