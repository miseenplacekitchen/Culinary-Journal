...
(function() {
  ...
  function ddListFormula(col, itemCount) {
    var endRow = Math.max(3, 2 + itemCount);
    return 'DD!$' + col + '$3:$' + col + '$' + endRow;
  }
  ...
  function writeDdListColumn(ws, listKey, items) {
    ...
    var i;
    for (i = 0; i < clearTo; i++) {
      ws.getCell(3 + i, colNum).value = i < items.length ? items[i] : null;
    }
    try {
      var table = ws.getTable(spec.table);
      if (table) {
        table.ref = spec.col + '2:' + spec.col + (2 + items.length);
        table.commit();
      }
    } catch (_) { /* table resize optional */ }
  }
  ...
  function writeDishIndexData(ws, headers, rows, rowValues) {
    ...
    try {
      var table = ws.getTable(DATA_TABLE_NAME);
      if (table) {
        var lastCol = colLetter(START_COL + headers.length - 1);
        var lastRow = Math.max(START_ROW + 1, START_ROW + rows.length);
        table.ref = 'B' + START_ROW + ':' + lastCol + lastRow;
        table.commit();
      }
    } catch (_) { /* table resize optional */ }
  }

  function applyDishIndexValidations(ws, headers, lists) {
    if (!ws || !ws.dataValidations) return;
    var dataStart = START_ROW + 1;
    var dataEnd = VALIDATION_DATA_END;
    headers.forEach(function(h, i) {
      var listKey = HEADER_LIST_KEY[h];
      if (!listKey || !DD_LIST[listKey]) return;
      var items = lists[listKey] || [];
      if (!items.length) return;
      var dishCol = colLetter(START_COL + i);
      var isTag = !!TAG_HEADERS[h];
      ws.dataValidations.add(dishCol + dataStart + ':' + dishCol + dataEnd, {
        type: 'list',
        allowBlank: true,
        formulae: [ddListFormula(DD_LIST[listKey].col, items.length)],
  ...
  async function buildWorkbookFromTemplate(opts) {
    ...
    applyDishIndexValidations(ws, headers, lists);
