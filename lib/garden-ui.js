...
(function (global) {
  ...
  function renderKitchenBridge(ingredients) {
    ...
    if (!ings.length) {
      return '<div class="gj-kitchen"><h3>From garden to kitchen</h3>' +
        '<p class="gj-kitchen-note">No kitchen link yet for this species. When ready, connect it to a governed ingredient in Admin → Garden Management → Kitchen links.</p></div>';
    }
  ...
  function renderGuildSection(guilds) {
    if (!guilds || !guilds.length) return '';
    var cards = guilds.map(function (g) {
      var memberList = (g.members || []).map(function (m) {
        return '<li>' + esc(m.name) + (m.role ? ' <span style="color:var(--text-muted)">— ' + esc(m.role) + '</span>' : '') + '</li>';
      }).join('');
      return '<div class="gj-guild-card" style="padding:16px;border:1px solid var(--border);border-radius:12px;background:var(--surface)">' +
        '<h3 style="font-family:Cormorant Garamond,serif;font-size:1.25rem;margin:0 0 8px">' + esc(g.name) + '</h3>' +
        (g.description ? '<p style="font-size:13px;color:var(--text-mid);line-height:1.6;margin:0 0 10px">' + esc(g.description) + '</p>' : '') +
        '<ul style="margin:0;padding-left:18px;font-size:13px;color:var(--text-high);line-height:1.7">' + memberList + '</ul></div>';
    }).join('');
    return '<section class="gj-wrap" style="margin-top:32px;padding-top:24px;border-top:1px solid var(--border)">' +
      '<h2 style="font-family:Cormorant Garamond,serif;font-size:1.6rem;margin:0 0 8px">Plant guilds</h2>' +
      '<p class="gj-section-lead">Named polycultures — companions that grow well together in your climate.</p>' +
      '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px;margin-top:16px">' + cards + '</div></section>';
  }
  ...
  global.GardenUI = {
    ...
    renderGuildSection: renderGuildSection,
