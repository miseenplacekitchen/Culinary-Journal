...
(function (global) {
  ...
  function renderKitchenBridge(ingredients) {
    ...
    return '<div class="gj-kitchen"><h3>From garden to kitchen</h3>' +
      ...
      '<p class="gj-kitchen-note">Garden v4: each <strong>variety / cultivar</strong> will match its own kitchen ingredient where needed (e.g. cherry tomato vs sauce tomato).</p></div>';
  }

  function renderVarietiesSection(plant) {
    var hint = plant && plant.variety_cultivar ? esc(plant.variety_cultivar) : 'Cultivar profiles are being imported from Variety Assessments.';
    return '<div class="gj-varieties"><div class="gj-varieties-soon">' +
      '<p style="margin:0 0 10px"><strong>Varieties for your climate</strong> — coming in Garden v4.</p>' +
      '<p style="margin:0 0 10px">' + hint + '</p>' +
      '<p style="margin:0;font-size:12px;color:var(--text-muted)">Heirloom, open-pollinated, hybrid, and indigenous cultivars will list here with care tuned per climate — each matched to the right kitchen ingredient.</p></div></div>';
  }
  ...
  function renderPlantProfile(data, opts) {
    ...
    var main =
      ...
      '<div class="gj-section"><h2>Varieties</h2>' + renderVarietiesSection(p) + '</div>' +
    ...
    return '<div class="gj-wrap">' +
      ...
      '<button type="button" class="gj-btn gj-btn-filled" id="gp-add">Add to My Garden</button>' +
  ...
  function renderTaskCard(task) {
    ...
    return '<div class="gj-task"><div class="gj-task-icon">' + emoji + '</div><div class="gj-task-body">' +
      '<div class="gj-task-title">' + esc(task.plant_name) + '</div>' +
  ...
  function renderMyPlantCard(p) {
    return '<div class="gj-side-card" style="margin-bottom:12px">' +
      '<a href="garden-plant.html?slug=' + encodeURIComponent(p.plant_slug || '') + '" style="font-family:Cormorant Garamond,serif;font-size:1.2rem;font-weight:600;color:var(--text-high);text-decoration:none">' +
      esc(p.plant_name) + '</a>' +
