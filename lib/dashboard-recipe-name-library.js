...
window.recipeNameLibrary = (function() {
  ...
  var _TABLE_COLS = [
    ...
    { key: 'recipe_name', label: 'Recipe Name', required: true, w: 140 },
    { key: 'native_name', label: 'Native', w: 110 },
    ...
    { key: 'origin_country', label: 'Country', w: 100 },
    { key: 'origin_state', label: 'State', w: 90 },
    { key: 'primary_ingredients', label: 'Hero Ingredient', array: true, w: 140 },
    { key: 'dietary_tags', label: 'Dietary', array: true, w: 110 },
    { key: 'meal_type_tags', label: 'Meal Type', array: true, w: 100 },
    { key: 'difficulty', label: 'Diff', w: 70 },
    { key: 'spice_level', label: 'Spice', w: 90 },
    { key: 'cooking_style', label: 'Style', w: 90 },
    { key: 'prep_time_minutes', label: 'Prep', w: 55 },
    { key: 'servings', label: 'Servings', w: 55 },
  ...
  function loadColVis() {
    _hiddenCols = new Set();
    try {
      var saved = JSON.parse(localStorage.getItem(_COL_VIS_KEY) || '[]');
    ...
    root.innerHTML =
      '<div class="di-panel">' +
        '<div class="di-panel-head">' +
          '<p class="di-intro"><strong>Dish Index v' + _SHELL_VERSION + '</strong> — Canonical dish registry (schema <code>' + _SCHEMA_VERSION + '</code>). Queue views filter the editorial pipeline; select rows for bulk actions. Import CSV for preview + chunked load. Ingredients and method stay in Submit a Recipe.</p>' +
    ...
    try {
      ...
      if (tbody) {
        tbody.innerHTML = '<tr><td colspan="' + cols.length + '" class="ap-empty-row">Error: ' + h(e.message || e) +
          '<br>Run SQL through <code>fix-dish-index-phase-abc.sql</code> (after list-filter), then refresh.</td></tr>';
      }
    }
  }
  ...
  function renderTable() {
    ...
    table.querySelector('thead').innerHTML = '<tr>' + cols.map(function(c) {
      ...
      if (c.key === 'select') return '<th class="' + sc.trim() + '" style="width:28px"><input type="checkbox" id="rnl-select-all"></th>';
      if (c.fixed || c.readonly) return '<th class="' + sc.trim() + '">' + h(c.label) + '</th>';
      var arrow = _sort.column === c.key ? (_sort.direction === 'asc' ? ' ▲' : ' ▼') : '';
      return '<th class="rnl-sort' + sc + '" data-col="' + h(c.key) + '" style="cursor:pointer;white-space:nowrap">' + h(c.label) + arrow + '</th>';
    }).join('') + '</tr>';
    tbody.innerHTML = '';
    if (!_rows.length) {
  ...
  function renderCell(r, c) {
    ...
    if (c.key === 'actions') {
      ...
      var archiveBtn = archived
        ? '<button class="ap-mini-btn rnl-restore" style="color:#4caf76">Restore</button>'
        : '<button class="ap-mini-btn rnl-delete" style="color:#dc5050">Archive</button>';
      return '<td><div class="rm-row-actions">' +
        ...
        print +
        (archived ? '' : '<button class="ap-mini-btn rnl-link">Link</button>') +
        archiveBtn +
      '</div></td>';
    }
    if (c.key === 'dish_code') {
      ...
      return '<td class="' + sc.trim() + '" style="font-family:ui-monospace,monospace;font-size:11px;white-space:nowrap">' + h(code) + driftMark + '</td>';
    }
    if (c.key === 'recipe_name') {
      var w = c.w ? 'min-width:' + c.w + 'px;' : 'min-width:120px;';
      return '<td class="' + sc.trim() + '"><div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">' +
        completenessChip(r) +
        '<input class="rnl-edit" data-field="recipe_name" value="' + h(r.recipe_name || '') + '" style="' + w + 'flex:1;padding:5px 7px;background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text-high);font-size:12px">' +
      '</div></td>';
    }
    ...
    if (c.key === 'research_status') return '<td>' + selectCellLabeled(c.key, _RESEARCH, _RESEARCH_LABELS, r[c.key] || 'idea_only', '') + '</td>';
    if (c.key === 'content_status') return '<td>' + selectCellLabeled(c.key, _CONTENT, _CONTENT_LABELS, r[c.key] || 'not_started', '') + '</td>';
    if (c.key === 'difficulty') return '<td>' + selectCell(c.key, _DIFFICULTY.filter(Boolean), r[c.key], 'Difficulty...') + '</td>';
    if (c.key === 'spice_level') return '<td>' + selectCell(c.key, _SPICE, r[c.key] || 'Not Applicable', '') + '</td>';
