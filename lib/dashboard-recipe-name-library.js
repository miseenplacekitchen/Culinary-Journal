...
window.recipeNameLibrary = (function() {
    ...
    try {
      ...
      if (tbody) {
        tbody.innerHTML = '<tr><td colspan="' + _TABLE_COLS.length + '" class="ap-empty-row">Error: ' + h(e.message || e) +
          '<br>Run <code>database/sql/fix-recipe-name-library.sql</code>, <code>fix-dish-index-columns.sql</code>, <code>fix-dish-index-ops.sql</code>, then <code>fix-dish-index-list-filter.sql</code> in Supabase, then refresh.</td></tr>';
      }
    }
  }
  ...
  function renderCell(r, c) {
    ...
    var w = c.w ? 'min-width:' + c.w + 'px;' : 'min-width:80px;';
    var title = c.array ? ' title="Semicolon-separated"' : '';
    return '<td><input class="rnl-edit" data-field="' + h(c.key) + '" value="' + h(val) + '"' + title + ' style="' + w + 'width:100%;padding:5px 7px;background:var(--bg);border:1px solid var(--border);border-radius:6px;color:var(--text-high);font-size:12px"></td>';
  }
