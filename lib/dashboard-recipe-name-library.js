...
window.recipeNameLibrary = (function() {
  ...
  var _QUEUE_WORKFLOW_KEYS = [
    'idea_only', 'needs_research', 'ready_unlinked', 'ready_linked',
    'verified_unlinked', 'verified_linked', 'linked_drift', 'other'
  ];
  ...
  var _QUEUE_PRESETS = [
    ...
    { id: 'archived', label: 'Archived only', archived: true }
  ];
  ...
  var _TABLE_COLS = [
    ...
    { key: 'difficulty', label: 'Difficulty', combo: true, w: 120 },
    { key: 'spice_level', label: 'Spice', combo: true, w: 120 },
    { key: 'sweet_level', label: 'Sweet', combo: true, w: 120 },
  ...
  var _CSV_HEADERS = [
    ...
    'Research Status', 'Content Status', 'Linked Recipe ID', 'Linked Recipe', 'Visibility', 'Active', 'Notes'
  ];
  ...
  function colFiltersPayload() {
    var out = {};
    Object.keys(_colFilters).forEach(function(k) {
      if (_colFilters[k]) out[k] = _colFilters[k];
    });
  ...
  function listFilters() {
    return {
      ...
      p_col_filters: colFiltersPayload()
    };
  }
  ...
  function levelFieldItems(selectKey) {
    if (rf().levelOptions) return rf().levelOptions(selectKey);
    return (rf()[selectKey] || []).map(function(v) {
      return { value: v, label: v ? v : '— Not set' };
    });
  }
    ...
    root.innerHTML =
      '<div class="di-panel">' +
        '<div class="di-panel-head">' +
          '<p class="di-intro"><strong>Dish Index v' + _SHELL_VERSION + '</strong> — Canonical dish registry (schema <code>' + _SCHEMA_VERSION + '</code>). <strong>All active</strong> is what you see in the table by default; workflow pills partition those rows. <strong>Archived only</strong> is separate (hidden from the table until you open that pill). Dropdowns are searchable — click × to clear.</p>' +
          '<div class="di-actions">' +
            '<button type="button" class="ing-add-btn" id="rnl-add-btn">+ Add dish</button>' +
            '<button type="button" class="ing-add-btn" id="rnl-import-btn" style="background:none;border:1px solid var(--accent);color:var(--accent)">Import Excel / CSV</button>' +
            '<button type="button" class="ing-add-btn" id="rnl-export-btn" style="background:none;border:1px solid var(--accent);color:var(--accent)">Export Excel</button>' +
            ...
            '<input type="file" id="rnl-file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" style="display:none">' +
          '</div>' +
        '</div>' +
        '<div class="di-section-label">Queue</div>' +
        '<div id="rnl-queue-row" class="di-queue-row"></div>' +
        '<p id="rnl-queue-hint" class="di-queue-hint"></p>' +
  ...
  function workflowQueueSum() {
    var s = 0;
    _QUEUE_WORKFLOW_KEYS.forEach(function(k) {
      if (_queueCounts[k] != null) s += _queueCounts[k];
    });
    return s;
  }

  function renderQueueHint() {
    var el = document.getElementById('rnl-queue-hint');
    if (!el) return;
    var active = _queueCounts.all != null ? _queueCounts.all : 0;
    var archived = _queueCounts.archived != null ? _queueCounts.archived : 0;
    var sum = workflowQueueSum();
    var total = active + archived;
    var parts = [active + ' active in table'];
    if (archived) parts.push(archived + ' archived (hidden — click Archived only)');
    parts.push(total + ' total in registry');
    if (sum !== active) {
      parts.push('workflow sum ' + sum + ( _queueCounts.other ? ' (includes ' + _queueCounts.other + ' other)' : ''));
    } else {
      parts.push('Idea + Needs research + Ready + Verified + Drift = ' + sum);
    }
  ...
  function renderQueuePills() {
    ...
    if (!row) return;
    var activePresets = _QUEUE_PRESETS.filter(function(q) { return !q.archived; });
    var archivedPreset = _QUEUE_PRESETS.find(function(q) { return q.archived; });
    function pillHtml(q) {
      var ck = _QUEUE_COUNT_KEYS[q.id];
      var n = ck && _queueCounts[ck] != null ? _queueCounts[ck] : null;
      var suffix = n != null ? ' (' + n + ')' : '';
      var cls = 'ap-filter-btn rnl-queue-pill' + (q.archived ? ' rnl-queue-archived' : '');
      return '<button type="button" class="' + cls + '" data-queue="' + h(q.id) + '" title="' +
        (q.archived ? 'Archived dishes — not included in All active' : 'Filter active dishes') + '">' +
        h(q.label) + suffix + '</button>';
    }
    row.innerHTML = activePresets.map(pillHtml).join('') +
      (archivedPreset ? '<span class="di-queue-sep" aria-hidden="true"></span>' + pillHtml(archivedPreset) : '');
  ...
  function bindShell() {
    ...
    if (table && !table.dataset.sortBound) {
      ...
      table.addEventListener('click', function(e) {
        if (e.target.closest('.rnl-col-filter')) return;
    ...
    if (table && !table.dataset.colFilterBound) {
      table.dataset.colFilterBound = '1';
      var cft;
      table.addEventListener('input', function(e) {
        var inp = e.target.closest('.rnl-col-filter');
        if (!inp) return;
        e.stopPropagation();
        clearTimeout(cft);
        ...
        cft = setTimeout(function() {
          ...
          if (v) _colFilters[col] = v;
          else delete _colFilters[col];
          _page = 1;
          loadRows(col);
        }, 350);
      });
      table.addEventListener('keydown', function(e) {
        if (e.key === 'Enter' && e.target.closest('.rnl-col-filter')) {
          e.preventDefault();
          clearTimeout(cft);
          var inp = e.target;
          var col = inp.dataset.col;
    ...
    if (file) file.addEventListener('change', importFile);
  ...
  async function loadRows(refocusCol) {
    ...
    var focusEnd = null;
    if (refocusCol) {
      var active = document.querySelector('.rnl-col-filter[data-col="' + refocusCol + '"]');
      if (active) {
    ...
    try {
      ...
      loadQueueCounts();
      if (refocusCol) {
        var inp = document.querySelector('.rnl-col-filter[data-col="' + refocusCol + '"]');
        if (inp) {
          inp.focus();
          try { inp.setSelectionRange(focusStart, focusEnd); } catch (_) {}
        }
      }
    } catch (e) {
      if (tbody) {
        tbody.innerHTML = '<tr><td colspan="' + cols.length + '" class="ap-empty-row">Error: ' + h(e.message || e) +
          '<br>Run Dish Index SQL through step 10 for column filters — see <code>database/manifest.json</code>.</td></tr>';
      }
    }
  }

  function isColFilterable(c) {
    return c.key !== 'select' && c.key !== 'actions';
  }

  function colsKey(cols) {
    return cols.map(function(c) { return c.key; }).join('|');
  }

  function renderTableHead(force) {
    var cols = visibleTableCols();
    var table = document.getElementById('rnl-table');
    if (!table) return;
    var key = colsKey(cols);
    if (!force && _headColsKey === key && table.querySelector('.di-head-filters')) return;
    _headColsKey = key;
    var labelRow = cols.map(function(c) {
      ...
      return '<th class="rnl-sort' + sc + '" data-col="' + h(c.key) + '" style="cursor:pointer;white-space:nowrap;' + w + '" title="Click to sort">' +
        h(c.label) + '<span style="opacity:0.45;font-size:10px;margin-left:3px">' + (arrow || '⇅') + '</span></th>';
    }).join('');
    var filterRow = cols.map(function(c) {
      var sc = stickyClass(c.key);
      if (!isColFilterable(c)) return '<th class="di-filter-th' + sc + '"></th>';
      var val = h(_colFilters[c.key] || '');
      return '<th class="di-filter-th' + sc + '">' +
        '<input type="text" class="rnl-col-filter ap-search" data-col="' + h(c.key) + '" value="' + val + '" placeholder="Filter" title="Filter ' + h(c.label) + '">' +
      '</th>';
    }).join('');
    table.querySelector('thead').innerHTML =
      '<tr class="di-head-labels">' + labelRow + '</tr>' +
      '<tr class="di-head-filters">' + filterRow + '</tr>';
  }
  ...
  function renderTableBody() {
    ...
    tbody.innerHTML = '';
    if (!_rows.length) {
      tbody.innerHTML = '<tr><td colspan="' + cols.length + '" class="ap-empty-row">No dishes match your filters.</td></tr>';
      return;
    }
  ...
  function syncDiStickyOffsets() {
    ...
    var head = table.querySelector('thead tr.di-head-labels');
    if (!head) return;
  ...
  function renderCell(r, c) {
    ...
    if (c.key === 'difficulty') {
      return '<td>' + (diEd().comboCell ? diEd().comboCell(c.key, levelFieldItems('DIFFICULTY'), r[c.key] || '', 'Difficulty…') : '') + '</td>';
    }
    if (c.key === 'spice_level') {
      return '<td>' + (diEd().comboCell ? diEd().comboCell(c.key, levelFieldItems('SPICE'), r[c.key] || 'Not Applicable', 'Spice…') : '') + '</td>';
    }
    if (c.key === 'sweet_level') {
      return '<td>' + (diEd().comboCell ? diEd().comboCell(c.key, levelFieldItems('SWEET'), r[c.key] || 'Not Applicable', 'Sweet…') : '') + '</td>';
    }
    if (c.tags) {
  ...
  function exportRow(r) {
    var x = (window.dishIndexExport && window.dishIndexExport.normalizeExportRow)
      ? window.dishIndexExport.normalizeExportRow(r)
      : r;
    var tag = window.dishIndexExport && window.dishIndexExport.exportTagText;
    return [
      _SCHEMA_VERSION, r.dish_code, r.id, r.recipe_name, r.native_name, arrText(r.alternate_names),
      r.category, r.sub_category, x.division,
      r.origin_continent, r.origin_country, r.origin_state, r.origin_locality,
      arrText(r.primary_ingredients),
      tag ? tag(r.dietary_tags, 'dietary_tags') : arrText(r.dietary_tags),
      tag ? tag(r.health_tags, 'health_tags') : arrText(r.health_tags),
      tag ? tag(r.meal_type_tags, 'meal_type_tags') : arrText(r.meal_type_tags),
      tag ? tag(r.occasion_tags, 'occasion_tags') : arrText(r.occasion_tags),
      tag ? tag(r.style_tags, 'style_tags') : arrText(r.style_tags),
      tag ? tag(r.flavor_profile_tags, 'flavor_profile_tags') : arrText(r.flavor_profile_tags),
      ...
      r.servings, r.servings_unit || 'people',
      x.difficulty, r.spice_level, r.sweet_level, x.cooking_style,
      ...
      r.linked_recipe_name || '', x.visibility || r.visibility || 'Private',
      (window.dishIndexExport && window.dishIndexExport.exportActive)
        ? window.dishIndexExport.exportActive(r)
        : (r.is_active === false || r.is_active === 'false' ? 'false' : 'true'),
      r.notes
    ];
  }

  function rowToExportValues(r) {
    return exportRow(r);
  }

  function exportListContext() {
    var subs = {};
    var divs = {};
    var servingsUnits = {};
    _taxRows.forEach(function(row) {
      if (row.subcategory_name) subs[row.subcategory_name] = true;
      if (row.division_name) divs[row.division_name] = true;
    });
    var continents = [];
    if (window.tcjOrigin && typeof window.tcjOrigin.allContinents === 'function') {
      continents = window.tcjOrigin.allContinents();
    } else if (window.CD) {
      continents = Object.keys(window.CD);
    }
    return {
      categories: _cats.slice(),
      subCategories: Object.keys(subs),
      divisions: Object.keys(divs),
      continents: continents,
      countries: _allCountries.slice(),
      servingsUnits: Object.keys(servingsUnits),
      visibility: _VISIBILITY.slice(),
      sourceTypes: _SOURCE_TYPES.slice(),
      research: _RESEARCH.slice(),
      content: _CONTENT.slice()
    };
  }

  function rowToCsvLine(r) {
    return rowToExportValues(r).map(csvCell).join(',');
  }
  ...
  async function exportCsv() {
    ...
    try {
      if (!_cats.length) await loadCatsAndTaxonomy();
      if (!_allCountries.length && window.tcjOrigin && typeof window.tcjOrigin.allCountries === 'function') {
        _allCountries = window.tcjOrigin.allCountries();
      }
      var all = await fetchAllRows();
      if (!window.dishIndexExport || !window.ExcelJS) {
        throw new Error('Excel export not loaded — hard refresh the dashboard.');
      }
      var blob = await window.dishIndexExport.buildWorkbook({
        headers: _CSV_HEADERS,
        rows: all,
        rowValues: rowToExportValues,
        schemaVersion: _SCHEMA_VERSION,
        context: exportListContext()
      });
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.href = url;
      a.download = 'dish-index-' + new Date().toISOString().slice(0, 10) + '.xlsx';
  ...
  async function previewImportRows(rows) {
    rows = rows.map(function(r) {
      return (window.dishIndexExport && window.dishIndexExport.normalizeImportRow)
        ? window.dishIndexExport.normalizeImportRow(r)
        : r;
    });
    var backdrop = document.getElementById('rnl-import-backdrop');
  ...
  async function importFile(e) {
    ...
    try {
      if (name.endsWith('.xlsx')) {
        if (!window.dishIndexExport || !window.dishIndexExport.parseExcelFile) {
          throw new Error('Excel import not loaded — hard refresh the dashboard.');
        }
        var xrows = await window.dishIndexExport.parseExcelFile(file, _SCHEMA_VERSION);
        await previewImportRows(xrows);
        return;
      }
