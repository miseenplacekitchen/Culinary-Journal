...
window.recipeNameLibrary = (function() {
  ...
  var _TABLE_COLS = [
    ...
    { key: 'native_name', label: 'Native Title', w: 110 },
    ...
    { key: 'origin_country', label: 'Country', select: true, w: 130 },
    { key: 'origin_state', label: 'State', select: true, w: 130 },
    ...
    { key: 'source_type', label: 'Source', select: true, w: 120 },
    { key: 'visibility', label: 'Visibility', select: true, w: 100 },
  ...
  function renderTable() {
    ...
    table.querySelector('thead').innerHTML = '<tr>' + cols.map(function(c) {
      ...
      if (c.key === 'dish_code') return '<th class="' + sc.trim() + '" style="min-width:88px">DI#</th>';
      if (c.key === 'recipe_name') return '<th class="' + sc.trim() + '" style="min-width:200px">Recipe Name</th>';
  ...
  function renderCell(r, c) {
    ...
    if (c.key === 'category') return '<td>' + selectCell(c.key, catsWithEmoji(), r[c.key], 'Category...') + '</td>';
    ...
    if (c.key === 'origin_country') return '<td>' + selectCell(c.key, countriesForSelect(), r[c.key], 'Country...') + '</td>';
    if (c.key === 'origin_state') return '<td>' + selectCell(c.key, statesForRow(r.origin_country), r[c.key], 'State...') + '</td>';
    if (c.key === 'source_type') return '<td>' + selectCellLabeled(c.key, _SOURCE_TYPES, _SOURCE_LABELS, r[c.key] || 'Original', 'Source...') + '</td>';
    if (c.key === 'visibility') return '<td>' + selectCell(c.key, _VISIBILITY, r[c.key] || 'Private', 'Visibility...') + '</td>';
  ...
  async function saveCell(el) {
    ...
    if (field === 'origin_country' || field === 'origin_state') {
