...
(function (root, factory) {
  ...
  var EXTRACTOR_VERSION = '2.0.0-wave2';
  ...
  function extractArticleTextFromHtml(html, host) {
    ...
    var fragment = extractArticleHtml(html, strat.strategy);
    var raw = fragment ? htmlFragmentToText(fragment) : '';
    if (raw.startsWith('INGREDIENTS\n') || raw.indexOf('\nMETHOD\n') > 0) {
      return raw;
    }
    var trimmed = raw.length > 150 ? trimBlogRecipeText(raw)
      : trimBlogRecipeText(htmlFragmentToText(html.slice(0, 120000)));
    var seg = segmentArticleText(trimmed);
    return seg.normalizedText || (RecipeImportCore ? RecipeImportCore.normalizeRecipeImportText(trimmed) : trimmed);
  }
  ...
  function extractRawArticleText(html, host) {
    ...
    var fragment = extractArticleHtml(html, strat.strategy);
    var raw = fragment ? htmlFragmentToText(fragment) : '';
    if (raw.length > 150) return trimBlogRecipeText(raw);
    return trimBlogRecipeText(htmlFragmentToText(String(html || '').slice(0, 120000)));
  }
  ...
  function buildImportPayload(opts) {
    ...
    var strat = resolveHostStrategy(host);
    var warnings = [];
    var rawArticleText = extractRawArticleText(html, host);
    var articleText = extractArticleTextFromHtml(html, host);
    var analysis = analyzeJsonLdRecipe(recipe);
    var extractor = strat.strategy;
    var mergeMode = false;
    var pasteText = articleText;
    var meta = analysis.meta || {};

    if (analysis.isTrustworthy) {
      pasteText = jsonLdToPasteText(analysis);
      extractor = 'jsonld';
    } else if (analysis.hasRecipe && articleText.length > 80) {
      var merged = mergeJsonLdWithArticle(recipe, articleText);
    ...
    var seg = segmentArticleText(pasteText);
    return {
      ok: true,
      url: opts.url || '',
      host: strat.host,
