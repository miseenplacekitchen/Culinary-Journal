...
(function (root, factory) {
  ...
  var EXTRACTOR_VERSION = '2.3.0-checklist-complete';
  ...
  function wprmItemToLine(innerHtml) {
    return htmlFragmentToText(innerHtml).replace(/\s+/g, ' ').trim();
  }
  ...
  function extractWprmListItems(html, classHint) {
    ...
    while ((lm = liRe.exec(m[1])) !== null) {
      var t = wprmItemToLine(lm[1]);
      if (t.length > 1) items.push(t);
    }
    return items;
  }

  function extractWprmItems(html, kind) {
    var scopes = [];
    var plugin = extractPluginRecipeHtml(html);
    if (plugin.length > 80) scopes.push(plugin);
    scopes.push(html);
    var seen = {};
    var out = [];
    ...
    scopes.forEach(function (scope) {
      if (kind === 'ingredient') {
        var ingLi = /<li[^>]*class="[^"]*wprm-recipe-ingredient[^"]*"[^>]*>([\s\S]*?)<\/li>/gi;
        var im;
        while ((im = ingLi.exec(scope)) !== null) push(wprmItemToLine(im[1]));
        if (!out.length) extractWprmListItems(scope, 'wprm-recipe-ingredient').forEach(push);
      } else {
        var instrDiv = /<div[^>]*class="[^"]*wprm-recipe-instruction-text[^"]*"[^>]*>([\s\S]*?)<\/div>/gi;
        var dm;
        while ((dm = instrDiv.exec(scope)) !== null) push(wprmItemToLine(dm[1]));
        var instrLi = /<li[^>]*class="[^"]*wprm-recipe-instruction[^"]*"[^>]*>([\s\S]*?)<\/li>/gi;
        var lm;
        while ((lm = instrLi.exec(scope)) !== null) push(wprmItemToLine(lm[1]));
        if (!out.length) extractWprmListItems(scope, 'wprm-recipe-instruction').forEach(push);
      }
    });
    return out;
  }
  ...
  function wprmHtmlToStructuredText(html) {
    var ings = extractWprmItems(html, 'ingredient');
    var steps = extractWprmItems(html, 'instruction');
  ...
  function extractWprmPasteText(html) {
    var text = wprmHtmlToStructuredText(html);
  ...
  function prepareArticlePasteText(html, host) {
    ...
    if (strat.strategy === 'wprm') {
      var wprmPaste = extractWprmPasteText(html);
      if (wprmPaste) {
        var wprmNorm = (RecipeImportCore && RecipeImportCore.normalizeRecipeImportText)
          ? RecipeImportCore.normalizeRecipeImportText(wprmPaste)
          : wprmPaste;
        return { trimmed: wprmPaste, normalized: wprmNorm, strategy: strat, extractorHint: 'wprm' };
      }
    }
  ...
  function extractJsonLdRecipe(html) {
    ...
    while ((m = re.exec(html)) !== null) {
      ...
      } catch (_) { TcjErr.warn('lib/recipe-import-extract.js:342', _); }
    }
    return null;
  }
  ...
  function buildImportPayload(opts) {
    ...
    if (prep.extractorHint === 'wprm') {
      seg = segmentArticleText(prep.normalized);
      pasteText = seg.normalizedText || prep.normalized;
      extractor = 'wprm';
    } else if (analysis.isTrustworthy || (strat.strategy === 'jsonld-first' && analysis.isComplete)) {
      pasteText = jsonLdToPasteText(analysis);
      extractor = 'jsonld';
      seg = segmentArticleText(pasteText);
      if (strat.strategy === 'jsonld-first') {
        warnings.push('Used recipe schema from page (jsonld-first host)');
      }
      ...
      if (strat.strategy === 'wprm' && seg.methCount < 2) {
        warnings.push('WPRM blocks not found — fell back to article text; review before submitting');
      }
      if (strat.strategy === 'jsonld-first' && !analysis.hasRecipe) {
        warnings.push('No recipe schema on page — paste manually if import looks incomplete');
      }
    }

    if (!seg) seg = segmentArticleText(pasteText);

    var conf = (RecipeImportCore && RecipeImportCore.computeImportConfidence)
      ? RecipeImportCore.computeImportConfidence(seg.ingCount, seg.method || [], warnings, seg.ingredients || [])
      : { score: null, allowEnrich: false, warnings: warnings };
    ...
    return {
      ...
      importQuality: {
        ...
        review_required: !conf.allowEnrich,
        enrich_allowed: conf.allowEnrich,
        warnings: conf.warnings || warnings
      }
    };
  }
  ...
  return {
    ...
    extractWprmPasteText: extractWprmPasteText,
    extractWprmItems: extractWprmItems,
    looksLikeStructuredRecipe: function (text) {
      return RecipeImportCore && RecipeImportCore.looksLikeStructuredRecipe
        ? RecipeImportCore.looksLikeStructuredRecipe(text)
        : false;
    },
    splitBundledCaption: function (text) {
      return RecipeImportCore && RecipeImportCore.splitBundledCaption
        ? RecipeImportCore.splitBundledCaption(text)
        : text;
    },
