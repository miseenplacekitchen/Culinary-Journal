...
window.bulkRecipeEditor = (function() {
  ...
  function bulkCatMatch(rowCat, cat) {
    return String(rowCat || '').trim() === String(cat || '').trim();
  }
  ...
  async function bulkLoadCategories() {
    ...
    try {
      if (typeof tcjFetchCategories === 'function') {
        var rows = await tcjFetchCategories();
        _bulkCategoryNames = (rows || []).map(function(r) { return r.name; }).filter(Boolean);
      }
    } catch (e) {
      console.warn('bulk categories', e);
    }
  }
  ...
  function bulkFillAssignDropdowns() {
    ...
    if (catSel.dataset.init !== '1') {
      ...
      var cats = _bulkCategoryNames.slice();
  ...
  function bulkStartEdit(td, recipe, col) {
    ...
    if (field === 'category') {
      bulkSelectEdit(td, origVal, _bulkCategoryNames.slice(), save);
  ...
  window.exportBulkPrintStudio = function(mode) {
    var ids = Array.from(_bulkSelected);
    if (!ids.length) { alert('Select at least one recipe first.'); return; }
    var collection = encodeURIComponent('Bulk Editor');
    if (mode === 'preview') {
      if (ids.length === 1) {
        window.open('print-studio.html?id=' + encodeURIComponent(ids[0]), '_blank');
      } else {
        window.open('print-studio.html?ids=' + encodeURIComponent(ids.join(',')) + '&collection=' + collection, '_blank');
      }
      return;
    }
    if (mode !== 'pdf' && mode !== 'pptx') return;
    var btn = document.getElementById('bulk-print-' + mode + '-btn');
    var label = mode === 'pdf' ? 'Download PDF' : 'Download PPTX';
    if (btn) { btn.disabled = true; btn.textContent = mode === 'pdf' ? 'Generating PDF…' : 'Generating PPTX…'; }
    var iframe = document.getElementById('bulk-print-export-frame');
    if (!iframe) {
      iframe = document.createElement('iframe');
      iframe.id = 'bulk-print-export-frame';
      iframe.title = 'Print export';
      iframe.style.cssText = 'position:fixed;width:0;height:0;border:0;visibility:hidden;pointer-events:none';
      document.body.appendChild(iframe);
    }
    var done = function(e) {
      if (!e.data || e.data.type !== 'tcj-print-export-done') return;
      window.removeEventListener('message', done);
      if (btn) { btn.disabled = false; btn.textContent = label; }
    };
    window.addEventListener('message', done);
    var url = 'print-studio.html?ids=' + encodeURIComponent(ids.join(',')) +
      '&collection=' + collection + '&export=' + mode + '&silent=1';
    iframe.src = url;
    setTimeout(function() {
      window.removeEventListener('message', done);
      if (btn && btn.disabled) { btn.disabled = false; btn.textContent = label; }
    }, 120000);
  };
