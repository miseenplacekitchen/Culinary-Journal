...
(function() {
  ...
  function bulkTaxSubs(cat) {
    ...
    _bulkTaxonomyRows.forEach(function(r) {
      if (r.subcategory_category === cat && r.subcategory_name) names[r.subcategory_name] = true;
    });
  ...
  function bulkTaxDivs(cat, sub) {
    var names = [];
    _bulkTaxonomyRows.forEach(function(r) {
      if (r.subcategory_category === cat && r.subcategory_name === sub && r.division_name) {
  ...
  window.loadBulkRecipesTab = async function() {
  ...
  async function renderBulkTaxonomyBackfill() {
    ...
    try {
      ...
      _bulkTaxonomyRows.forEach(function(r) {
        ...
        if (!cats[r.subcategory_category]) cats[r.subcategory_category] = {};
        cats[r.subcategory_category][r.subcategory_name] = true;
      });
  ...
  function bulkSelectEdit(td, origVal, options, save) {
    ...
    sel.addEventListener('blur', function() { save(sel.value); td.textContent = sel.value || '—'; });
